...
The later [upgrade authorization slice](NOVOVM_NATIVE_NONCE_UPGRADE_AUTHORIZATION_V1.md)
adds `test_native_nonce_upgrade_authorization`; validator consent is separate
from source canonicality, AOEM publication and activation.
...
adds `test_native_seal_new_view`, extending the contract to 50 without unlocking
candidate height locks. The subsequent
[new-view candidate admission slice](NOVOVM_NATIVE_SEAL_NEW_VIEW_ADMISSION_V1.md)
adds `test_native_seal_new_view_admission`.
The serializer, preflight and node-runtime locksets now contain 51 required
fields in the same order. The frozen contract includes rejection when the
workspace, nonce-identity, checkpoint, staging, authorization, source-QC, new-view
or new-view admission evidence field
