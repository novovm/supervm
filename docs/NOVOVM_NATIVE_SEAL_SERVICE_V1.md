...
本机已完成[真实 AOEM 主节点 prepare QC 联调](NOVOVM_NATIVE_SEAL_MAIN_PROCESS_V1.md)：
四个固定成员、初始负责人离线，2/4 不取得 QC、3/4 实际取得并持久保存 QC，
活动主节点正常重启恢复；这不是最终确认，测试需要独占回环 443 且默认不随 CI 执行。
