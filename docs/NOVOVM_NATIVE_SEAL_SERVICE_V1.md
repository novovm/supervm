...
2026-09-26 收尾复核：此前完整主线门禁 51/51、封印回归 105/105、启动 CLI
4/4 通过，但这不代表全库测试全绿。额外运行现有 binary 的
`native_execution_pipeline_tests` 得到 26/31：旧默认测试库缺 nonce 身份版本；
隔离目录后仍暴露请求内持久路径已被禁止、共享进程中 Propagated 被错误期待为
Pending 等旧夹具问题。尝试的子进程夹具改造还出现执行等待，未通过验收，已撤回。
这些问题待独立修复；未删除旧数据库、未放松生产路径或 nonce 校验。
