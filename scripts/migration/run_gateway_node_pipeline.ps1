...
if ($EnableReconcileDaemon) {
    if ([string]::IsNullOrWhiteSpace($ReconcileSenderAddress)) {
        throw "EnableReconcileDaemon requires ReconcileSenderAddress"
    }
    if ($SkipGatewayStart) {
        throw "EnableReconcileDaemon requires gateway process started by current pipeline (SkipGatewayStart=false)"
    }
}
...
if (-not $SkipGatewayStart) {
    ...
    $gatewayEmbedReconcile = if ($EnableReconcileDaemon) { "1" } else { "0" }
    $gatewayEnv = @{
        "NOVOVM_GATEWAY_BIND" = $GatewayBind
        "NOVOVM_GATEWAY_SPOOL_DIR" = $SpoolDir
        "NOVOVM_GATEWAY_MAX_REQUESTS" = "$GatewayMaxRequests"
        "NOVOVM_GATEWAY_EMBED_RECONCILE_DAEMON" = $gatewayEmbedReconcile
        "NOVOVM_RECONCILE_ENABLED" = $gatewayEmbedReconcile
    }
    if ($EnableReconcileDaemon) {
        $gatewayEnv["NOVOVM_GATEWAY_RECONCILE_SENDER_ADDRESS"] = $ReconcileSenderAddress
        $gatewayEnv["NOVOVM_GATEWAY_RECONCILE_RPC_ENDPOINT"] = $ReconcileRpcEndpoint
        $gatewayEnv["NOVOVM_GATEWAY_RECONCILE_INTERVAL_SECONDS"] = "$ReconcileIntervalSeconds"
        $gatewayEnv["NOVOVM_GATEWAY_RECONCILE_RESTART_DELAY_SECONDS"] = "$ReconcileRestartDelaySeconds"
        $gatewayEnv["NOVOVM_GATEWAY_RECONCILE_REPLAY_MAX_PER_PAYOUT"] = "$ReconcileReplayMaxPerPayout"
        $gatewayEnv["NOVOVM_GATEWAY_RECONCILE_REPLAY_COOLDOWN_SEC"] = "$ReconcileReplayCooldownSec"
        $gatewayEnv["NOVOVM_RECONCILE_SENDER_ADDRESS"] = $ReconcileSenderAddress
        $gatewayEnv["NOVOVM_RECONCILE_RPC_ENDPOINT"] = $ReconcileRpcEndpoint
        $gatewayEnv["NOVOVM_RECONCILE_INTERVAL_SECONDS"] = "$ReconcileIntervalSeconds"
        $gatewayEnv["NOVOVM_RECONCILE_RESTART_DELAY_SECONDS"] = "$ReconcileRestartDelaySeconds"
        $gatewayEnv["NOVOVM_RECONCILE_REPLAY_MAX_PER_PAYOUT"] = "$ReconcileReplayMaxPerPayout"
        $gatewayEnv["NOVOVM_RECONCILE_REPLAY_COOLDOWN_SEC"] = "$ReconcileReplayCooldownSec"
    }
    ...
    if ($gatewayProc.HasExited) {
        if ($LeanIo) {
            throw "edge gateway exited unexpectedly (lean_io=true)"
        }
        throw "edge gateway exited unexpectedly. check logs: $gatewayOut ; $gatewayErr"
    }
}

Write-Host "pipeline_in: spool_dir=$SpoolDir poll_ms=$PollMs supervisor_poll_ms=$SupervisorPollMs idle_exit_seconds=$IdleExitSeconds run_once=$RunOnce gateway_started=$(-not $SkipGatewayStart) use_node_watch_mode=$UseNodeWatchMode lean_io=$LeanIo node_watch_batch_max_files=$NodeWatchBatchMaxFiles reconcile_daemon=$EnableReconcileDaemon requeued_from_processing=$requeued"
...
try {
    if ($UseNodeWatchMode) {
        $effectiveIdleExit = $IdleExitSeconds
        if ($RunOnce -and $effectiveIdleExit -le 0) {
            $effectiveIdleExit = 1
        }
        $nodeOut = Join-Path $LogRoot "novovm-node.watch.stdout.log"
        $nodeErr = Join-Path $LogRoot "novovm-node.watch.stderr.log"
        $nodeEnv = @{
            "NOVOVM_NODE_MODE" = "full"
            "NOVOVM_EXEC_PATH" = "ffi_v2"
            "NOVOVM_D1_INGRESS_MODE" = "ops_wire_v1"
            "NOVOVM_OPS_WIRE_DIR" = $SpoolDir
            "NOVOVM_OPS_WIRE_WATCH" = "1"
            "NOVOVM_OPS_WIRE_WATCH_POLL_MS" = "$PollMs"
            "NOVOVM_OPS_WIRE_WATCH_IDLE_EXIT_SECONDS" = "$effectiveIdleExit"
            "NOVOVM_OPS_WIRE_WATCH_BATCH_MAX_FILES" = "$NodeWatchBatchMaxFiles"
            "NOVOVM_TX_REPEAT_COUNT" = "1"
        }
        if (-not $LeanIo) {
            $nodeEnv["NOVOVM_OPS_WIRE_WATCH_DONE_DIR"] = $DoneRoot
            $nodeEnv["NOVOVM_OPS_WIRE_WATCH_FAILED_DIR"] = $FailedRoot
        } else {
            $nodeEnv["NOVOVM_OPS_WIRE_WATCH_DROP_FAILED"] = "1"
        }
        $nodeEnvState = Push-ProcessEnv -Environment $nodeEnv
        $nodeProc = $null
        try {
            if ($LeanIo) {
            ...
            if ($null -eq $nodeProc) {
                throw "start novovm-node watch mode returned null: $NodeExe"
            }
        } finally {
            Pop-ProcessEnv -State $nodeEnvState
        }

        Write-Host "pipeline_watch_in: spool_dir=$SpoolDir done_root=$DoneRoot failed_root=$FailedRoot idle_exit_seconds=$effectiveIdleExit lean_io=$LeanIo"

        while ($true) {
            if ($gatewayProc -ne $null -and $gatewayProc.HasExited) {
                if ($nodeProc -ne $null -and -not $nodeProc.HasExited) {
                    Stop-Process -Id $nodeProc.Id -Force -ErrorAction SilentlyContinue
                }
                throw "edge gateway exited unexpectedly during pipeline watch mode"
            }
            if ($nodeProc -ne $null -and $nodeProc.HasExited) {
                if ($nodeProc.ExitCode -ne 0) {
                    if ($LeanIo) {
                        throw "novovm-node watch mode exited with code=$($nodeProc.ExitCode) (lean_io=true)"
                    }
                    throw "novovm-node watch mode exited with code=$($nodeProc.ExitCode). check logs: $nodeOut ; $nodeErr"
                }
                break
            }
            Start-Sleep -Milliseconds $SupervisorPollMs
        }

        Write-Host "pipeline_watch_out: node_exit_code=0 spool_dir=$SpoolDir done_root=$DoneRoot failed_root=$FailedRoot lean_io=$LeanIo"
    } else {
        while ($true) {
            if ($gatewayProc -ne $null -and $gatewayProc.HasExited) {
