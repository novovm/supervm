...
if (-not $OutputDir) {
    $OutputDir = Join-Path $RepoRoot "artifacts\migration\foreign-rate-source-gate"
}
...
function Invoke-NodeProbe {
    ...
    if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
        try { $proc.Kill() } catch {}
        throw "foreign rate source probe timed out after ${TimeoutSeconds}s"
    }
...
function Parse-ForeignSourceOutLine {
    param([string]$Text)

    $line = ($Text -split "`r?`n" | Where-Object { $_ -match "^governance_market_foreign_source_out:" } | Select-Object -Last 1)
    if (-not $line) { return $null }
    $payload = ($line -replace "^governance_market_foreign_source_out:\s*", "")
    $pairs = @{}
    ...
    if (-not $pairs.ContainsKey("proposal_id") -or -not $pairs.ContainsKey("foreign_source_applied")) {
    ...
    return [ordered]@{
        ...
        foreign_source_applied = [bool]::Parse($pairs["foreign_source_applied"])
        source = $pairs["source"]
        quote_spec_applied = [bool]::Parse($pairs["quote_spec_applied"])
...
function Start-OneShotForeignRateFeedServer {
    param(
    ...
    return Start-Job -ScriptBlock {
        param($Port, $QuoteSpec, $SignatureSha256)
        ...
        try {
            ...
            try {
                ...
                $escaped = $QuoteSpec.Replace('\', '\\').Replace('"', '\"')
                if ($SignatureSha256) {
                    $body = "{`"quote_spec`":`"$escaped`",`"signature_sha256`":`"$SignatureSha256`"}"
                } else {
                    $body = "{`"quote_spec`":`"$escaped`"}"
                }
    ...
    } -ArgumentList $Port, $QuoteSpec, $SignatureSha256
}
...
$tests = @(
    [ordered]@{
        key = "foreign_rate_spec_ok"
        crate = "web30-core"
        workdir = Join-Path $RepoRoot "vendor\web30-core"
        args = @("test", "--quiet", "test_configurable_rate_provider_apply_quote_spec_ok", "--", "--exact")
    },
    [ordered]@{
        key = "foreign_rate_invalid_rate_reject"
        crate = "web30-core"
        workdir = Join-Path $RepoRoot "vendor\web30-core"
        args = @("test", "--quiet", "test_configurable_rate_provider_reject_invalid_rate", "--", "--exact")
    },
    [ordered]@{
        key = "foreign_rate_invalid_slippage_reject"
        crate = "web30-core"
        workdir = Join-Path $RepoRoot "vendor\web30-core"
        args = @("test", "--quiet", "test_configurable_rate_provider_reject_invalid_slippage", "--", "--exact")
    },
    [ordered]@{
        key = "foreign_rate_processing_configurable_provider"
        crate = "web30-core"
        workdir = Join-Path $RepoRoot "vendor\web30-core"
        args = @("test", "--quiet", "test_foreign_payment_processing_with_configured_provider", "--", "--exact")
    },
    [ordered]@{
        key = "market_engine_foreign_path_regression"
        crate = "novovm-consensus"
        workdir = Join-Path $RepoRoot "crates\novovm-consensus"
        args = @("test", "--quiet", "test_market_engine_apply_policy")
    },
    [ordered]@{
        key = "market_engine_foreign_external_quote_spec"
        crate = "novovm-consensus"
        workdir = Join-Path $RepoRoot "crates\novovm-consensus"
        args = @("test", "--quiet", "test_foreign_rate_source_external_with_quote_spec")
    },
    [ordered]@{
        key = "market_engine_foreign_external_fallback"
        crate = "novovm-consensus"
        workdir = Join-Path $RepoRoot "crates\novovm-consensus"
        args = @("test", "--quiet", "test_foreign_rate_source_external_missing_quote_fallback")
    },
    [ordered]@{
        key = "market_engine_foreign_invalid_quote_reject"
        crate = "novovm-consensus"
        workdir = Join-Path $RepoRoot "crates\novovm-consensus"
        args = @("test", "--quiet", "test_foreign_rate_source_reject_invalid_quote_spec")
    }
)
...
$nodeBuildStdout = Join-Path $OutputDir "foreign_source_node_build.stdout.log"
$nodeBuildStderr = Join-Path $OutputDir "foreign_source_node_build.stderr.log"
...
$results += [ordered]@{
    key = "foreign_source_node_build_ok"
    crate = "novovm-node"
...
if ($nodeBuild.exit_code -eq 0) {
    ...
    if (-not $nodeExe) {
        $results += [ordered]@{
            key = "foreign_source_node_binary_found"
            crate = "novovm-node"
            workdir = $nodeCrateDir
        ...
        $signatureKey = "foreign_rate_signing_key_v1"
        $feedQuoteSpecA = "BTC:120000:90,ETH:6000:70,USDT:10:20"
        $feedQuoteSpecB = "BTC:120000:90,ETH:6000:70,USDT:10:20"
        ...
        $sigA = Get-Sha256Hex -Value "foreign_rate_feed_v1|quote_spec=$feedQuoteSpecA|$signatureKey"
        ...
        try {
            $positiveProbe = Invoke-NodeProbe `
                ...
                -EnvVars @{
                    ...
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_URLS = "http://127.0.0.1:$feedPortA/quote,http://127.0.0.1:$feedPortB/quote"
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_MIN_SOURCES = "2"
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_TIMEOUT_MS = "1200"
        ...
        $positiveStdout = Join-Path $OutputDir "foreign_source_external_feed_probe.stdout.log"
        $positiveStderr = Join-Path $OutputDir "foreign_source_external_feed_probe.stderr.log"
        ...
        $positiveLine = Parse-ForeignSourceOutLine -Text $positiveProbe.output
        $positivePass = [bool](
            $positiveProbe.exit_code -eq 0 -and
            $positiveLine -and
            $positiveLine.parse_ok -and
            $positiveLine.foreign_source_applied -and
            $positiveLine.source -eq "foreign_external_feed_http_v1" -and
            $positiveLine.quote_spec_applied -and
        ...
        $results += [ordered]@{
            key = "foreign_source_external_feed_probe_ok"
            crate = "novovm-node"
            workdir = $RepoRoot
            command = "NOVOVM_NODE_MODE=governance_market_policy_probe + foreign external feed quote"
            pass = $positivePass
        ...
        try {
            $fallbackProbe = Invoke-NodeProbe `
                ...
                -EnvVars @{
                    ...
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_MODE = "external_feed"
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_SOURCE_NAME = "foreign_external_feed_http_v1"
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_URLS = "http://127.0.0.1:$feedPortFallback/quote,http://127.0.0.1:65530/quote"
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_MIN_SOURCES = "2"
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_TIMEOUT_MS = "300"
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_SIGNATURE_REQUIRED = "1"
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_SIGNATURE_KEY = $signatureKey
        ...
        $fallbackStdout = Join-Path $OutputDir "foreign_source_external_feed_fallback.stdout.log"
        $fallbackStderr = Join-Path $OutputDir "foreign_source_external_feed_fallback.stderr.log"
        ...
        $fallbackLine = Parse-ForeignSourceOutLine -Text $fallbackProbe.output
        $fallbackPass = [bool](
            $fallbackProbe.exit_code -eq 0 -and
            $fallbackLine -and
            $fallbackLine.parse_ok -and
            $fallbackLine.foreign_source_applied -and
            $fallbackLine.source -eq "foreign_external_feed_http_v1" -and
        ...
        $results += [ordered]@{
            key = "foreign_source_external_feed_fallback_ok"
            crate = "novovm-node"
            workdir = $RepoRoot
            command = "NOVOVM_NODE_MODE=governance_market_policy_probe + foreign multisource insufficient fallback"
            pass = $fallbackPass
        ...
        try {
            $strictFailProbe = Invoke-NodeProbe `
                ...
                -EnvVars @{
                    ...
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_MIN_SOURCES = "1"
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_TIMEOUT_MS = "300"
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_SIGNATURE_REQUIRED = "1"
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_SIGNATURE_KEY = $signatureKey
                    NOVOVM_GOV_MARKET_FOREIGN_RATE_FEED_STRICT = "1"
                }
        } finally {
            if ($badFeedJob) {
                try {
        ...
        $strictFailStdout = Join-Path $OutputDir "foreign_source_external_feed_strict_reject.stdout.log"
        $strictFailStderr = Join-Path $OutputDir "foreign_source_external_feed_strict_reject.stderr.log"
        ...
        $strictFailPass = [bool](
            ...
            ($strictFailProbe.output -match "foreign_rate_feed_fetch_failed") -and
            ($strictFailProbe.output -match "signature mismatch")
        )
        $results += [ordered]@{
            key = "foreign_source_external_feed_signature_strict_reject_ok"
            crate = "novovm-node"
            workdir = $RepoRoot
            command = "NOVOVM_NODE_MODE=governance_market_policy_probe + foreign strict bad signature reject"
            pass = $strictFailPass
...
$summaryJson = Join-Path $OutputDir "foreign-rate-source-gate-summary.json"
$summaryMd = Join-Path $OutputDir "foreign-rate-source-gate-summary.md"
$summary | ConvertTo-Json -Depth 8 | Set-Content -Path $summaryJson -Encoding UTF8

$md = @(
    "# Foreign Rate Source Gate Summary"
    ""
...
Write-Host "foreign rate source gate summary:"
Write-Host "  pass: $($summary.pass)"
Write-Host "  error_reason: $($summary.error_reason)"
Write-Host "  summary_json: $summaryJson"

if (-not $allPass) {
    throw "foreign rate source gate FAILED: $errorReason"
}

Write-Host "foreign rate source gate PASS"
