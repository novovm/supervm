...
This shell only forwards parameters to `novovmctl rollout`.
#>
...
if (-not (Test-ArgPresent -Args $ForwardArgs -Names @("-Action", "--action"))) {
    $baseArgs += @("--action", "upgrade")
}

Invoke-NovovmctlForward -RepoRoot $repoRoot -Subcommand "rollout" -BaseArgs $baseArgs -IncomingArgs $ForwardArgs
