...
mod tests {
    ...
    fn official_staticcall_oog_no_commit_state_fixture_v1() -> serde_json::Value {
        serde_json::from_str(include_str!(
            "../tests/fixtures/ethereum-official-state-subset/staticcall-oog-no-commit.json"
        ))
        .expect("decode official STATICCALL OOG no-commit state fixture")
    }
    ...
    fn official_call_output_state_fixture_v1() -> serde_json::Value {
        serde_json::from_str(include_str!(
            "../tests/fixtures/ethereum-official-state-subset/call-output.json"
        ))
    ...
    fn official_call_depth_oog_state_fixture_v1() -> serde_json::Value {
        serde_json::from_str(include_str!(
            "../tests/fixtures/ethereum-official-state-subset/call-depth-oog.json"
        ))
    ...
    fn official_log4_oog_receipt_state_fixture_v1() -> serde_json::Value {
        serde_json::from_str(include_str!(
            "../tests/fixtures/ethereum-official-state-subset/log4-oog-receipt.json"
        ))
        .expect("decode official LOG4/OOG receipt state fixture")
    }
    ...
    #[test]
    fn official_state_fixture_staticcall_oog_no_commit_grouped_projection_v1() {
        let fixture = official_staticcall_oog_no_commit_state_fixture_v1();
        assert_eq!(fixture["source"]["repo"].as_str(), Some("ethereum/tests"));
        assert_eq!(
            fixture["source"]["fixtureFormat"].as_str(),
            Some("state_test")
        ...
        let cases = fixture["cases"]
            .as_array()
            .expect("STATICCALL OOG no-commit cases");
        assert_eq!(cases.len(), 10);

        let mut fixture_keys = std::collections::HashSet::new();
        let mut fixture_hashes = std::collections::HashSet::new();
        let mut txbytes_seen = std::collections::HashSet::new();
        let mut create_oog_cases = 0usize;
        let mut nested_call_oog_cases = 0usize;
        let mut touched_account_count = 0usize;
        let mut code_preservation_facts = 0usize;
        let mut gas_by_label = std::collections::HashMap::new();
        ...
        for case in cases {
            let label = json_str(&case["label"], "case.label");
            let mode = json_str(&case["mode"], "case.mode");
            assert_eq!(json_str(&case["kind"], "case.kind"), "static-oog-no-commit");
            assert_eq!(case["statusOk"].as_bool(), Some(false));
            assert_eq!(case["failureClass"].as_str(), Some("out_of_gas"));
            assert!(
                json_str(&case["fixtureKey"], "case.fixtureKey")
                    .contains("GeneralStateTests/stStaticCall/static_"),
                "official STATICCALL OOG no-commit case must come from stStaticCall"
            );
            assert!(
                json_str(&case["fixtureKey"], "case.fixtureKey").contains("-fork_[Cancun-Prague]"),
                "official STATICCALL OOG no-commit case must be the Cancun/Prague projection"
            );
            assert!(
                fixture_keys.insert(json_str(&case["fixtureKey"], "case.fixtureKey")),
                "official STATICCALL OOG no-commit fixture key must be unique"
            );
            fixture_hashes.insert(json_str(&case["fixtureHash"], "case.fixtureHash"));
            assert!(
                json_str(&case["generatedTestHash"], "case.generatedTestHash").starts_with("0x")
            );
            assert!(json_str(&case["sourceHash"], "case.sourceHash").starts_with("0x"));
            assert!(json_str(&case["postHash"], "case.postHash").starts_with("0x"));
            assert_eq!(
                json_str(&case["logsHash"], "case.logsHash"),
                empty_logs_hash
            );
            ...
            assert_eq!(
                case["preTargetStorageFacts"], case["postTargetStorageFacts"],
                "official STATICCALL OOG target storage must be preserved"
            );
            code_preservation_facts += 1;
            ...
            txbytes_seen.insert(json_str(&case["txbytes"], "case.txbytes"));
            let recovered_sender = recover_raw_evm_tx_sender_m0(&raw_tx)
                .expect("official STATICCALL OOG raw sender recovery")
                .expect("official STATICCALL OOG recovered sender");
            assert_eq!(recovered_sender, decode_hex_bytes(sender));
            let fields =
                translate_raw_evm_tx_fields_m0(&raw_tx).expect("official STATICCALL OOG tx decode");
            let tx = tx_ir_from_raw_fields_m0(&fields, &raw_tx, recovered_sender, 1);

            let data = decode_hex_bytes(json_str(&case["data"], "case.data"));
            if data.is_empty() {
                assert_eq!(
                    tx.tx_type,
                    TxType::Transfer,
                    "raw empty-calldata STATICCALL OOG fixture tx is state-agnostic before execution"
                );
            } else {
                assert_eq!(
                    tx.tx_type,
                    TxType::ContractCall,
                    "raw calldata STATICCALL OOG fixture tx is contract-call-shaped before state promotion"
                );
            }
            assert_eq!(tx.from, decode_hex_bytes(sender));
            assert_eq!(tx.to, Some(decode_hex_bytes(to)));
            ...
            let mut adapter = NovoVmAdapter::new(ChainConfig {
                chain_type: ChainType::EVM,
                chain_id: 1,
                name: format!("official-staticcall-oog-no-commit-{label}"),
                enabled: true,
                custom_config: None,
            });
            adapter.initialize().expect("init");
            assert!(
                adapter
                    .verify_transaction(&tx)
                    .expect("verify official STATICCALL OOG raw tx"),
                "official STATICCALL OOG txbytes must verify through raw EVM path"
            );
            ...
            assert_eq!(
                expected_gas_used, tx.gas_limit,
                "official STATICCALL OOG no-commit cases must consume full gas limit"
            );
                ...
                .expect("case.touchedAccounts")
            {
                ...
                assert_eq!(
                    account["preStorageFacts"], account["postStorageFacts"],
                    "official STATICCALL OOG touched storage must be preserved"
                );
                code_preservation_facts += 1;
                let code_hash = if pre_code_bytes == 0 {
                    None
            ...
            assert_eq!(
                NovoVmAdapter::effective_execution_tx_v1(&tx, &runtime_state).tx_type,
                TxType::ContractCall,
                "target code in pre-state must route STATICCALL OOG fixture tx as contract call execution"
            );

            let mut artifact = sample_aoem_artifact(&tx, false, [0x94; 32], None);
            artifact.gas_used = expected_gas_used;
            artifact.cumulative_gas_used = expected_gas_used;
            artifact.effective_gas_price = Some(gas_price as u64);
            artifact.receipt_type = None;
            ...
            let outcome = adapter
                ...
                .expect("execute official STATICCALL OOG no-commit fixture projection");
            ...
            let bal = outcome
                .observed_block_access_list
                .block_access_list
                .expect("official STATICCALL OOG observed BAL");
            let sender_entry = bal
                .0
                .iter()
                .find(|entry| entry.address.as_slice() == tx.from.as_slice())
            ...
            assert!(
                bal.0
                    ...
                    .filter(|entry| entry.address.as_slice() != tx.from.as_slice())
                    .all(|entry| entry.balance_changes.is_empty()),
                "zero-value STATICCALL OOG no-commit cases must not emit non-sender balance changes"
            );

            if mode.contains("CREATE") {
                create_oog_cases += 1;
            }
            if mode.contains("SECOND_LEVEL") || mode.contains("CALL_CALLCODE") {
                nested_call_oog_cases += 1;
            }
            assert!(
                gas_by_label
                    .insert(label.to_string(), expected_gas_used)
                    .is_none(),
                "duplicate official STATICCALL OOG no-commit label {label}"
            );
        }

        assert_eq!(fixture_keys.len(), 10);
        assert_eq!(fixture_hashes.len(), 10);
        assert_eq!(txbytes_seen.len(), 9);
        assert_eq!(create_oog_cases, 2);
        assert_eq!(nested_call_oog_cases, 3);
        assert_eq!(touched_account_count, 13);
        assert_eq!(code_preservation_facts, 23);
        assert_eq!(
            gas_by_label.get("staticCallContractToCreateContractAndCallItOog"),
            Some(&2_000_000)
        );
        assert_eq!(
            gas_by_label.get("staticZeroValueCallOogRevert"),
            Some(&1_350_000)
        );
        assert_eq!(gas_by_label.get("staticCheckCallCostOog"), Some(&22_000));
    }
    ...
    #[test]
    fn official_state_fixture_call_output_grouped_projection_v1() {
        let fixture = official_call_output_state_fixture_v1();
        assert_eq!(fixture["source"]["repo"].as_str(), Some("ethereum/tests"));
        assert_eq!(
            fixture["source"]["fixtureFormat"].as_str(),
            Some("state_test")
        ...
        let empty_logs_hash = json_str(&fixture["emptyLogsHash"], "emptyLogsHash");
        let cases = fixture["cases"].as_array().expect("CALL output cases");
        assert_eq!(cases.len(), 6);

        let mut success_cases = 0usize;
        let mut failure_cases = 0usize;
        let mut nested_commit_cases = 0usize;
        let mut nested_no_commit_cases = 0usize;
        let mut gas_by_label = std::collections::HashMap::new();
        let mut fixture_hashes = std::collections::HashSet::new();
        let mut post_hashes = std::collections::HashSet::new();
        let mut txbytes_seen = std::collections::HashSet::new();
        let mut target_storage_fact = None::<String>;
        ...
        for case in cases {
            ...
            assert!(
                json_str(&case["fixtureKey"], "case.fixtureKey")
                    .contains("GeneralStateTests/stCallCreateCallCodeTest/callOutput"),
                "official CALL output case must come from stCallCreateCallCodeTest/callOutput"
            );
            assert!(
                json_str(&case["fixtureKey"], "case.fixtureKey").contains("-fork_[Cancun-Prague]"),
                "official CALL output case must be the Cancun/Prague projection"
            );
            assert!(
                fixture_hashes.insert(json_str(&case["fixtureHash"], "case.fixtureHash")),
                "official CALL output fixture hash must be unique"
            );
            assert!(
                post_hashes.insert(json_str(&case["postHash"], "case.postHash")),
                "official CALL output post hash must be unique"
            );
            assert_eq!(
                json_str(&case["logsHash"], "case.logsHash"),
                empty_logs_hash
            ...
            assert_eq!(
                post_to_balance,
                pre_to_balance + value,
                "CALL output subset intentionally keeps top-level value transfer simple"
            );
            assert_eq!(
                post_nested_balance, pre_nested_balance,
                "selected CALL output subset has no nested value transfer"
            );
            ...
            txbytes_seen.insert(json_str(&case["txbytes"], "case.txbytes"));
            let recovered_sender = recover_raw_evm_tx_sender_m0(&raw_tx)
                .expect("official CALL output raw sender recovery")
                .expect("official CALL output recovered sender");
            assert_eq!(recovered_sender, decode_hex_bytes(sender));
            let fields =
                translate_raw_evm_tx_fields_m0(&raw_tx).expect("official CALL output tx decode");
            let tx = tx_ir_from_raw_fields_m0(&fields, &raw_tx, recovered_sender, 1);

            assert_eq!(
                tx.tx_type,
                TxType::Transfer,
                "raw empty-calldata CALL output fixture tx is state-agnostic before execution"
            );
            assert_eq!(tx.from, decode_hex_bytes(sender));
            assert_eq!(tx.to, Some(decode_hex_bytes(to)));
            assert_eq!(tx.value, value);
            ...
            let mut adapter = NovoVmAdapter::new(ChainConfig {
                chain_type: ChainType::EVM,
                chain_id: 1,
                name: format!("official-call-output-{label}"),
                enabled: true,
                custom_config: None,
            });
            adapter.initialize().expect("init");
            assert!(
                adapter
                    .verify_transaction(&tx)
                    .expect("verify official CALL output raw tx"),
                "official CALL output txbytes must verify through raw EVM path"
            );
            for historical_nonce in 0..pre_sender_nonce {
                let mut historical_tx = tx.clone();
                historical_tx.nonce = historical_nonce;
                adapter
                    .route_transaction_through_unified_account(&historical_tx)
                    .expect("prime official CALL output pre-state nonce");
            }
            ...
            runtime_state.set_account(
                ...
                AccountState {
                    balance: pre_to_balance,
                    nonce: pre_to_nonce,
                    code_hash: Some(vec![0xce; 32]),
                    storage_root: vec![0u8; 32],
                },
            );
            runtime_state.set_account(
                decode_hex_bytes(nested_target),
                AccountState {
            ...
            assert_eq!(
                NovoVmAdapter::effective_execution_tx_v1(&tx, &runtime_state).tx_type,
                TxType::ContractCall,
                "target code in pre-state must promote CALL output fixture tx to contract call execution"
            );

            let mut artifact = sample_aoem_artifact(&tx, true, [0x8b; 32], None);
            artifact.gas_used = expected_gas_used;
            artifact.cumulative_gas_used = expected_gas_used;
            artifact.effective_gas_price = Some(gas_price as u64);
            artifact.receipt_type = None;
            ...
            let outcome = adapter
                ...
                .expect("execute official CALL output fixture projection");
            ...
            assert_eq!(post_target_storage.len(), 1);
            let observed_target_fact = json_str(
                &case["postTargetStorageFacts"]["0x00"],
                "CALL output target slot0",
            )
            .to_string();
            if let Some(expected_fact) = target_storage_fact.as_ref() {
                assert_eq!(&observed_target_fact, expected_fact);
            } else {
                target_storage_fact = Some(observed_target_fact);
            }

            let nested_committed = case["nestedStorageCommitted"]
                .as_bool()
                .expect("case.nestedStorageCommitted bool");
            assert_eq!(nested_committed, !post_nested_storage.is_empty());
            match mode {
                "callOutputSuccessProjection" => {
                    success_cases += 1;
                    nested_commit_cases += 1;
                    assert!(nested_committed);
                    assert_eq!(
                        json_str(
                            &case["postNestedStorageFacts"]["0x00"],
                            "CALL output nested slot0"
                        ),
                        "0x02"
                    );
                }
                "callOutputFailureProjection" => {
                    failure_cases += 1;
                    nested_no_commit_cases += 1;
                    assert!(!nested_committed);
                    assert!(post_nested_storage.is_empty());
                }
                _ => panic!("unexpected official CALL output mode {mode}"),
            }
            assert!(
                gas_by_label
                    .insert(label.to_string(), expected_gas_used)
                    .is_none(),
                "duplicate official CALL output label {label}"
            );

            let bal = outcome
                .observed_block_access_list
                .block_access_list
                .expect("official CALL output observed BAL");
            let sender_entry = bal
                .0
                .iter()
                .find(|entry| entry.address.as_slice() == tx.from.as_slice())
            ...
            assert!(
                ...
                "nested CALL output state remains host/AOEM responsibility"
            );
        }

        assert_eq!(success_cases, 4);
        assert_eq!(failure_cases, 2);
        assert_eq!(nested_commit_cases, 4);
        assert_eq!(nested_no_commit_cases, 2);
        assert_eq!(
            target_storage_fact.expect("CALL output target storage fact"),
            "0x5e20a0453cecd065ea59c37ac63e079ee08998b6045136a8ce6635c7912ec0b6"
        );
        assert_eq!(
            txbytes_seen.len(),
            1,
            "selected official CALL output cases intentionally share one top-level txbytes and vary contract pre-state"
        );

        let gas = |label: &str| {
            *gas_by_label
                .get(label)
                .unwrap_or_else(|| panic!("missing official CALL output gas label {label}"))
        };
        assert_eq!(gas("callOutput1"), 67_856);
        assert_eq!(gas("callOutput2"), gas("callOutput1"));
        assert_eq!(gas("callOutput3"), gas("callOutput1"));
        assert_eq!(gas("callOutput3partial"), gas("callOutput1"));
        assert_eq!(gas("callOutput3Fail"), 95_744);
        assert_eq!(gas("callOutput3partialFail"), gas("callOutput3Fail"));
        assert!(gas("callOutput3Fail") > gas("callOutput3"));
    }
    ...
    #[test]
    fn official_state_fixture_call_depth_oog_grouped_projection_v1() {
        let fixture = official_call_depth_oog_state_fixture_v1();
        assert_eq!(fixture["source"]["repo"].as_str(), Some("ethereum/tests"));
        assert_eq!(
            fixture["source"]["fixtureFormat"].as_str(),
            Some("state_test")
        ...
        let empty_logs_hash = json_str(&fixture["emptyLogsHash"], "emptyLogsHash");
        let cases = fixture["cases"].as_array().expect("CALL depth/OOG cases");
        assert_eq!(cases.len(), 6);

        let mut balance_too_low_cases = 0usize;
        let mut depth_oog_cases = 0usize;
        let mut lose_gas_oog_cases = 0usize;
        let mut two_slot_storage_cases = 0usize;
        let mut three_slot_storage_cases = 0usize;
        let mut gas_by_label = std::collections::HashMap::new();
        let mut post_hashes = std::collections::HashSet::new();
        let mut txbytes_seen = std::collections::HashSet::new();
        ...
        for case in cases {
            ...
            assert!(
                json_str(&case["fixtureKey"], "case.fixtureKey")
                    .contains("GeneralStateTests/stCallCreateCallCodeTest/Call"),
                "official CALL depth/OOG case must come from stCallCreateCallCodeTest/Call*"
            );
            assert!(
                json_str(&case["fixtureKey"], "case.fixtureKey").contains("-fork_[Cancun-Prague]"),
                "official CALL depth/OOG case must be the Cancun/Prague projection"
            );
            assert!(
                post_hashes.insert(json_str(&case["postHash"], "case.postHash")),
                "official CALL depth/OOG post hash must be unique"
            );
            assert_eq!(
                json_str(&case["logsHash"], "case.logsHash"),
                empty_logs_hash
            ...
            assert_eq!(
                post_to_balance,
                pre_to_balance + value,
                "selected CALL depth/OOG cases keep top-level value flow directly attributable"
            );
            assert_eq!(post_to_nonce, pre_to_nonce);

            let raw_tx = decode_hex_bytes(json_str(&case["txbytes"], "case.txbytes"));
            assert!(txbytes_seen.insert(json_str(&case["txbytes"], "case.txbytes")));
            let recovered_sender = recover_raw_evm_tx_sender_m0(&raw_tx)
                .expect("official CALL depth/OOG raw sender recovery")
                .expect("official CALL depth/OOG recovered sender");
            assert_eq!(recovered_sender, decode_hex_bytes(sender));
            let fields =
                translate_raw_evm_tx_fields_m0(&raw_tx).expect("official CALL depth/OOG tx decode");
            let tx = tx_ir_from_raw_fields_m0(&fields, &raw_tx, recovered_sender, 1);

            assert_eq!(
                tx.tx_type,
                TxType::Transfer,
                "raw empty-calldata CALL depth/OOG fixture tx is state-agnostic before execution"
            );
            assert_eq!(tx.from, decode_hex_bytes(sender));
            assert_eq!(tx.to, Some(decode_hex_bytes(to)));
            assert_eq!(tx.value, value);
            ...
            let mut adapter = NovoVmAdapter::new(ChainConfig {
                chain_type: ChainType::EVM,
                chain_id: 1,
                name: format!("official-call-depth-oog-{label}"),
                enabled: true,
                custom_config: None,
            });
            adapter.initialize().expect("init");
            assert!(
                adapter
                    .verify_transaction(&tx)
                    .expect("verify official CALL depth/OOG raw tx"),
                "official CALL depth/OOG txbytes must verify through raw EVM path"
            );
            for historical_nonce in 0..pre_sender_nonce {
                let mut historical_tx = tx.clone();
                historical_tx.nonce = historical_nonce;
                adapter
                    .route_transaction_through_unified_account(&historical_tx)
                    .expect("prime official CALL depth/OOG pre-state nonce");
            }
            ...
            assert_eq!(
                NovoVmAdapter::effective_execution_tx_v1(&tx, &runtime_state).tx_type,
                TxType::ContractCall,
                "target code in pre-state must promote CALL depth/OOG fixture tx to contract call execution"
            );

            let mut artifact = sample_aoem_artifact(&tx, true, [0x8d; 32], None);
            artifact.gas_used = expected_gas_used;
            artifact.cumulative_gas_used = expected_gas_used;
            artifact.effective_gas_price = Some(gas_price as u64);
            artifact.receipt_type = None;
            ...
            let outcome = adapter
                ...
                .expect("execute official CALL depth/OOG fixture projection");
            ...
            match post_target_storage.len() {
                ...
                3 => three_slot_storage_cases += 1,
                other => panic!("unexpected CALL depth/OOG post storage slots {other}"),
            }

            match mode {
                "callDepthBalanceTooLowProjection" => {
                    balance_too_low_cases += 1;
                    assert_eq!(label, "Call1024BalanceTooLow");
                    assert_eq!(
                        json_str(&case["postTargetStorageFacts"]["0x00"], "depth slot0"),
                        "0x0401"
                    );
                    assert_eq!(
                        json_str(
                            &case["postTargetStorageFacts"]["0x01"],
                            "depth success marker"
                        ),
                        "0x01"
                    );
                }
                "callDepthOogProjection" => {
                    depth_oog_cases += 1;
                    assert_eq!(
                        json_str(
                            &case["postTargetStorageFacts"]["0x01"],
                    ...
                    assert!(
                        json_str(&case["postTargetStorageFacts"]["0x02"], "OOG gas slot")
                            .starts_with("0x")
                    );
                }
                "callLoseGasOogProjection" => {
                    lose_gas_oog_cases += 1;
                    assert_eq!(label, "CallLoseGasOOG");
                    assert_eq!(
                        json_str(&case["postTargetStorageFacts"]["0x00"], "lose gas slot0"),
                        "0x01"
                    );
                    assert_eq!(
                        json_str(&case["postTargetStorageFacts"]["0x02"], "lose gas slot2"),
                        "0x03e9"
                    );
                }
                _ => panic!("unexpected official CALL depth/OOG mode {mode}"),
            }
            assert!(
                gas_by_label
                    .insert(label.to_string(), expected_gas_used)
                    .is_none(),
                "duplicate official CALL depth/OOG label {label}"
            );

            let bal = outcome
                .observed_block_access_list
                .block_access_list
                .expect("official CALL depth/OOG observed BAL");
            let sender_entry = bal
                .0
                .iter()
                .find(|entry| entry.address.as_slice() == tx.from.as_slice())
        ...
        assert_eq!(balance_too_low_cases, 1);
        assert_eq!(depth_oog_cases, 4);
        assert_eq!(lose_gas_oog_cases, 1);
        assert_eq!(two_slot_storage_cases, 2);
        assert_eq!(three_slot_storage_cases, 4);
        assert_eq!(txbytes_seen.len(), 6);

        let gas = |label: &str| {
            *gas_by_label
                .get(label)
                .unwrap_or_else(|| panic!("missing official CALL depth/OOG gas label {label}"))
        };
        assert_eq!(gas("Call1024BalanceTooLow"), 7_481_800);
        assert_eq!(gas("Call1024OOGGas0"), 1_751_479);
        assert_eq!(gas("Call1024OOGGas1"), 1_716_608);
        assert_eq!(gas("Call1024OOGGas2"), 1_748_187);
        assert_eq!(gas("Call1024OOGGas3"), 1_745_038);
        assert_eq!(gas("CallLoseGasOOG"), 167_771);
        assert!(gas("Call1024BalanceTooLow") > gas("Call1024OOGGas0"));
        assert!(gas("Call1024OOGGas0") > gas("Call1024OOGGas2"));
        assert!(gas("Call1024OOGGas2") > gas("Call1024OOGGas3"));
        assert!(gas("Call1024OOGGas3") > gas("Call1024OOGGas1"));
        assert!(gas("Call1024OOGGas1") > gas("CallLoseGasOOG"));
    }
    ...
    #[test]
    fn official_state_fixture_log4_oog_receipt_grouped_projection_v1() {
        let fixture = official_log4_oog_receipt_state_fixture_v1();
        assert_eq!(fixture["source"]["repo"].as_str(), Some("ethereum/tests"));
        assert_eq!(
            fixture["source"]["fixtureFormat"].as_str(),
            Some("state_test")
        ...
        let empty_logs_hash = json_str(&fixture["emptyLogsHash"], "emptyLogsHash");
        let cases = fixture["cases"].as_array().expect("LOG4/OOG cases");
        assert_eq!(cases.len(), 10);

        let mut log4_success_cases = 0usize;
        let mut no_log_cases = 0usize;
        let mut topic_count_sum = 0usize;
        let mut empty_data_log_cases = 0usize;
        let mut non_empty_data_log_cases = 0usize;
        let mut storage_fact_cases = 0usize;
        let mut non_empty_logs_hashes = std::collections::HashSet::new();
        let mut gas_by_label = std::collections::HashMap::new();
        ...
        for case in cases {
            ...
            assert!(
                json_str(&case["fixtureKey"], "case.fixtureKey")
                    .contains("GeneralStateTests/VMTests/vmLogTest/log4.json"),
                "official LOG4/OOG case must come from VMTests/vmLogTest/log4.json"
            );
            assert!(
                json_str(&case["fixtureKey"], "case.fixtureKey").contains("-fork_[Cancun-Prague]"),
                "official LOG4/OOG case must be the Cancun/Prague projection"
            );
            assert!(
                json_str(&case["postHash"], "case.postHash").starts_with("0x"),
                "official LOG4/OOG case must carry post hash"
            );
            let logs_hash = json_str(&case["logsHash"], "case.logsHash");
            let logs_expected = case["logsExpected"]
                .as_bool()
            ...
            assert_eq!(logs_expected, logs_hash != empty_logs_hash);
            if logs_expected {
                non_empty_logs_hashes.insert(logs_hash.to_string());
            }
            ...
            assert!(
                value > 0,
                "selected official LOG4/OOG subset covers simple top-level value transfer"
            );
            assert_eq!(post_to_balance, pre_to_balance + value);

            let raw_tx = decode_hex_bytes(json_str(&case["txbytes"], "case.txbytes"));
            let recovered_sender = recover_raw_evm_tx_sender_m0(&raw_tx)
                .expect("official LOG4/OOG raw sender recovery")
                .expect("official LOG4/OOG recovered sender");
            assert_eq!(recovered_sender, decode_hex_bytes(sender));
            let fields =
                translate_raw_evm_tx_fields_m0(&raw_tx).expect("official LOG4/OOG tx decode");
            let tx = tx_ir_from_raw_fields_m0(&fields, &raw_tx, recovered_sender, 1);

            assert_eq!(
                tx.tx_type,
                TxType::ContractCall,
                "VMTests LOG4 fixture uses calldata and must decode as a contract call"
            );
            assert_eq!(tx.from, decode_hex_bytes(sender));
            assert_eq!(tx.to, Some(decode_hex_bytes(to)));
            assert_eq!(tx.value, value);
            ...
            let mut adapter = NovoVmAdapter::new(ChainConfig {
                chain_type: ChainType::EVM,
                chain_id: 1,
                name: format!("official-log4-oog-receipt-{label}"),
                enabled: true,
                custom_config: None,
            });
            adapter.initialize().expect("init");
            assert!(
                adapter
                    .verify_transaction(&tx)
                    .expect("verify official LOG4/OOG raw tx"),
                "official LOG4/OOG txbytes must verify through raw EVM path"
            );
            for historical_nonce in 0..pre_sender_nonce {
                let mut historical_tx = tx.clone();
                historical_tx.nonce = historical_nonce;
                adapter
                    .route_transaction_through_unified_account(&historical_tx)
                    .expect("prime official LOG4/OOG pre-state nonce");
            }
            ...
            assert_eq!(
                NovoVmAdapter::effective_execution_tx_v1(&tx, &runtime_state).tx_type,
                TxType::ContractCall,
                "target code in pre-state must promote LOG4/OOG fixture tx to contract call execution"
            );
            ...
            if logs_expected {
                ...
                let topics = (0..topic_count)
                    ...
                    .collect::<Vec<_>>();
                let data_kind = json_str(&case["logDataKind"], "case.logDataKind");
                let data = if matches!(data_kind, "emptyMem" | "memSizeZero") {
                    empty_data_log_cases += 1;
                    Vec::new()
                } else {
                    non_empty_data_log_cases += 1;
                    vec![0x4c, 0x4f, 0x47, 0x34]
                };
                artifact.event_logs = vec![AoemEventLogV1 {
                    emitter: tx.to.clone().expect("log emitter"),
                    topics,
                ...
                artifact.log_bloom = vec![0x45; AOEM_LOG_BLOOM_BYTES_V1];
            } else {
                artifact.event_logs.clear();
                artifact.log_bloom = vec![0u8; AOEM_LOG_BLOOM_BYTES_V1];
            }
            ...
            let outcome = adapter
                ...
                .expect("execute official LOG4/OOG receipt fixture projection");
            ...
            let post_storage_facts = case["postStorageFacts"]
                ...
                .expect("case.postStorageFacts");
            match mode {
                "log4SuccessProjection" => {
                    log4_success_cases += 1;
                    assert_eq!(outcome.artifact.event_logs.len(), 1);
                    assert_eq!(outcome.artifact.event_logs[0].topics.len(), 4);
                    assert!(outcome.artifact.log_bloom.iter().any(|byte| *byte != 0));
                    assert!(!post_storage_facts.is_empty());
                    assert_eq!(
                        json_str(&case["postStorageFacts"]["0x00"], "log4 storage fact"),
                        "0x600d"
                    );
                    storage_fact_cases += 1;
                    assert!(
                        runtime_state
                            .get_storage(&tx.from, b"aoem:last_event_logs")
                            .is_some(),
                ...
                "log4NoLogProjection" => {
                    ...
                    assert!(outcome.artifact.log_bloom.iter().all(|byte| *byte == 0));
                    assert_eq!(
                        json_str(&case["postStorageFacts"]["0x00"], "no-log storage fact"),
                        "0x0bad"
                    );
                    storage_fact_cases += 1;
                    assert!(
                        runtime_state
                            .get_storage(&tx.from, b"aoem:last_event_logs")
                            .is_none(),
                        "no-log official OOG cases must not persist AOEM logs"
                    );
                }
                _ => panic!("unexpected official LOG4/OOG mode {mode}"),
            }

            let bal = outcome
                .observed_block_access_list
                .block_access_list
                .expect("official LOG4/OOG observed BAL");
            let sender_entry = bal
                .0
                .iter()
                .find(|entry| entry.address.as_slice() == tx.from.as_slice())
            ...
            assert!(
                gas_by_label
                    .insert(label.to_string(), expected_gas_used)
                    .is_none(),
                "duplicate official LOG4 label {label}"
            );
        }

        assert_eq!(log4_success_cases, 8);
        assert_eq!(no_log_cases, 2);
        assert_eq!(topic_count_sum, 32);
        assert_eq!(empty_data_log_cases, 2);
        assert_eq!(non_empty_data_log_cases, 6);
        assert_eq!(storage_fact_cases, 10);
        assert_eq!(non_empty_logs_hashes.len(), 7);

        let gas = |label: &str| {
            *gas_by_label
                .get(label)
                .unwrap_or_else(|| panic!("missing official LOG4 gas label {label}"))
        };
        assert_eq!(gas("log4EmptyMem"), 30_717);
        assert_eq!(gas("log4MemSizeZero"), 30_741);
        assert_eq!(gas("log4NonEmptyMem"), 30_997);
        assert_eq!(gas("log4Log01"), 30_749);
        assert_eq!(gas("log4Log311"), 30_749);
        assert_eq!(gas("log4Caller"), 30_996);
        assert_eq!(gas("log4MaxTopic"), 30_773);
        assert_eq!(gas("log4Pc"), 30_745);
        assert_eq!(gas("log4MemStartTooHigh"), 78_750_373);
        assert_eq!(gas("log4MemSizeTooHigh"), 78_750_373);
        assert_eq!(gas("log4Log01"), gas("log4Log311"));
        assert_eq!(gas("log4MemStartTooHigh"), gas("log4MemSizeTooHigh"));
        assert!(gas("log4MemStartTooHigh") > gas("log4NonEmptyMem"));
    }
    ...
    fn evm_adapter_balance_fee_access_storage_surface_smoke_v1() {
        ...
        official_state_fixture_staticcall_oog_no_commit_grouped_projection_v1();
