...
mod tests {
    ...
    fn single_relay_forward_roundtrip() {
        ...
        let relay_client = RelayClient::new("relay-01".to_string(), "target-a".to_string(), &relay_server);

        let result = relay_client.forward_with("req-1".to_string(), b"ping".to_vec(), |target, payload| {
