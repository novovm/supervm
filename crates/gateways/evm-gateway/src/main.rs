...
fn ensure_eth_no_params(method: &str, params: &serde_json::Value) -> Result<()> {
    match params {
        ...
        _ => bail!("{method} does not accept parameters"),
    }
}
...
fn run_gateway_method(
    ...
    match method {
        ...
        "eth_baseFee" => {
            ensure_eth_no_params("eth_baseFee", params)?;
            let (fee_history, _) = run_gateway_method(
                router,
                eth_tx_index,
                evm_settlement_index_by_id,
                evm_settlement_index_by_tx,
            ...
            Ok((serde_json::Value::String(next_base_fee.to_string()), false))
        }
        "eth_feeHistory" => {
            let chain_id = param_as_u64_any_with_tx(params, &["chain_id", "chainId"])
                .unwrap_or(ctx.eth_default_chain_id);
