...
mod tests {
    ...
    fn make_sample(
        ...
        MainlineSoakSamplePointV1 {
            observed_unix_ms: ts,
            snapshot_updated_at_unix_ms: updated,
            execution_budget_hit_count: hits,
            execution_deferred_count: deferred,
            execution_time_slice_exceeded_count: time_slice,
            header_updates: headers,
            body_updates: bodies,
            sync_requests: sync,
            pending_depth: pending,
            hard_budget_per_tick: Some(hard_budget),
            target_budget_per_tick: Some(effective_budget),
            effective_budget_per_tick: Some(effective_budget),
            hard_time_slice_ms: Some(hard_slice),
            target_time_slice_ms: Some(effective_slice),
            effective_time_slice_ms: Some(effective_slice),
            target_reason: Some(reason.to_string()),
