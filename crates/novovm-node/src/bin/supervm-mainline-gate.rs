...
struct MainlineGate {
    ...
    test_native_seal_new_view_admission: bool,
...
fn main() -> Result<()> {
    ...
    let mut gate = MainlineGate {
        ...
        test_native_seal_new_view_admission: false,
    ...
    run_step(
        "test native candidate workspace persistence, isolation, and recovery",
        "cargo",
        &[
            "test",
            "-p",
    ...
    gate.test_native_candidate_workspace = true;

    run_step(
        "test native nonce identity V2 and offline legacy migration preflight",
        "cargo",
        &[
            "test",
            "-p",
    ...
    run_step(
        "test native nonce upgrade authorization and durable validator signing fence",
        "cargo",
        &[
            "test",
            "-p",
            "novovm-node",
            "--lib",
            "native_nonce_upgrade_authorization",
            "--",
            "--test-threads=1",
        ],
    )?;
    run_step(
        "test native nonce upgrade authorization verification CLI",
        "cargo",
        &[
            "test",
            "-p",
    ...
    run_step(
        "test native new-view candidate admission and mandatory signing guards",
        "cargo",
        &[
            "test",
            "-p",
            "novovm-node",
            "--lib",
            "native_seal_new_view_admission",
            "--",
            "--test-threads=1",
        ],
    )?;
    gate.test_native_seal_new_view_admission = true;
