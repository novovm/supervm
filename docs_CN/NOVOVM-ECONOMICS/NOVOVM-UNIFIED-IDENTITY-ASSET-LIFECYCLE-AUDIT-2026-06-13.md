...
- `ua_autoHealMappedAssets` 提供最小自动 reorg heal 执行入口：dry-run 报告候选、所需 policy 和当前 policy 是否会执行；`apply=true` 必须由 governance/Treasury policy 开启，开启后冻结 unsafe active/burn_pending live NETH；unsafe frozen NETH 的自动 rollback 还必须额外开启 `mapped_asset_auto_heal_rollback_enabled`。无论 freeze 还是 rollback，都不触发外部链释放、NOV mint 或赔付。
