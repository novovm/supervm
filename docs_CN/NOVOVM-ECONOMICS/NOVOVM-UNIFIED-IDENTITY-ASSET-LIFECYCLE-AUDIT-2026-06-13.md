...
- M2 bridge risk status v1 已接入主线只读入口 `nov_getM2BridgeRiskStatus`：聚合 `ua_getMappedFinalitySourceStatus`、Treasury `get_reserve_proof`、native mapped lock / burn / release / auto-heal policy，返回 `active / constrained / blocked` 风险口径；该入口只读，不创建账户、不写 UCA、不触发外部释放、不 mint NOV、不启动后台调度。
