...
不在范围：外部独立项目、外部钱包前端、真实 Ethereum lock 合约部署、安全审计
...
- M2 auto heal v1 已接入 `ua_autoHealMappedAssets` 和主线显式调度入口 `nov_runMappedAssetAutoHeal`：默认 dry-run 报告 unsafe source anchor，并对每个候选返回 `required_policy/policy_would_apply`；governance/Treasury policy 可通过主线 `nov_applyTreasuryPolicy(mapped_asset_reorg_response_policy)` 统一配置为 `report_only / freeze_only / freeze_and_rollback`。`apply=true` 必须同时满足 `scheduler_authorized=true` 和对应 policy；未授权 apply fail-closed。`freeze_only` 可自动冻结 active/burn_pending live NETH，扣减用户 native 可用余额并保留 Treasury reserve；`freeze_and_rollback` 额外允许把 unsafe frozen NETH 内部回滚为 `rejected` 并扣回 Treasury reserve。主线产品 smoke 已覆盖 live NETH 入账后 source anchor reorg -> explicit tick dry-run -> unauthorized apply fail-closed -> mainline governance policy freeze -> policy rollback。
...
- M2 bridge risk status v1 已接入主线只读入口 `nov_getM2BridgeRiskStatus`：聚合 `ua_getMappedFinalitySourceStatus`、Treasury `get_reserve_proof`、native mapped lock / burn / release / auto-heal policy，并计算 `account_m2_liability / treasury_reserve_balance / reserve_proof_amount` 覆盖关系，返回 `active / constrained / blocked` 风险口径；缺失或失效 reserve proof、缺失 governed lock contract、缺失 governed min confirmations、bridge/burn/release pause、M2 liability 超过 Treasury reserve 或 reserve proof cap 都按 `blocked` fail-closed；NETH treasury redeem 与非 NOV fee clearing 会复用该 NETH M2 risk gate，风险 blocked 时返回失败 receipt，不继续清算；finality source 处于 `open_unprotected` 时按 `constrained` 返回，并显式透出 `finality_gap=*` 原因；该入口只读，不创建账户、不写 UCA、不触发外部释放、不 mint NOV、不启动后台调度。该入口已新增 `operations_readiness`：即使 `risk_state=active`，也只表示 internal M2 credit / risk-gated Treasury settlement 可用，不表示完整 external bridge、外部链 release、常驻 scheduler、compensation policy 或 NOV emission policy 已 ready。
- 隐私能力状态查询 v1 已接入主线只读入口 `nov_getPrivacyCapabilityStatus`：返回 mainline read gate、AOEM RingCT、AOEM ZK 和 MLDSA 的分层能力状态；该入口不生成 proof、不泄露用户级 nAsset 余额、不创建第二账本，并明确 `MLDSA` 是账户/交易签名的抗量子能力，不是隐私交易系统。
- 隐私审计策略查询 v1 已接入主线只读入口 `nov_getPrivacyAuditPolicy`：固定受保护用户级 M2 查询面为 `nov_getAssetBalance`、`account_balance`、`account_assets`，默认 `redacted`，授权披露必须带 `privacy_disclosure` 并可用 `nov_verifyPrivacyDisclosureReceipt` 验证；该入口禁止 gateway bypass、禁止第二账本，不生成 RingCT/ZK proof。
...
- 隐私能力状态查询 v1 已落代码：`nov_getPrivacyCapabilityStatus` 返回 `read_gate_only / privacy_proof_capable`、RingCT/ZK prove/verify capability、MLDSA `pq_signature_only` role 和 M2 read redaction policy；该入口只读，不生成 proof，不泄露用户级 nAsset 余额。
- 隐私审计策略查询 v1 已落代码：`nov_getPrivacyAuditPolicy` 返回受保护用户级 M2 查询方法、默认 redaction、授权披露 receipt 要求、验证方法、gateway bypass 禁止、no second ledger 和 RingCT/ZK proof upgrade path；该入口只读，不生成 proof，不写账本。
- 授权披露 receipt v1 已落代码：授权的用户级 M2 查询会返回 `privacy_disclosure`，用于证明本次明细披露经过 mainline read gate，并用 `result_commitment_digest` 绑定本次授权响应；`nov_verifyPrivacyDisclosureReceipt` 已覆盖 `nov_getAssetBalance`、`account_balance`、`account_assets` 三类受保护查询，原响应通过、篡改响应失败；该 receipt 当前是审计票据，不是 RingCT/ZK proof。
...
- 这不是完整真实外部桥；live 模式的 receipt MPT proof 已要求锚定本地 runtime canonical finalized block，并可通过主线 `nov_applyTreasuryPolicy(mapped_lock_contract_address)` 固定治理 lock contract address。主线产品 smoke 已覆盖错误治理 lock contract 覆盖参数 expected address 并 fail-closed、正确治理 lock contract 后 live NETH/M2 credit 通过。`nov_setMappedHeaderSourcePolicy` 可约束 header source peer 和 `min_source_quorum`，且 quorum 已按同一 `block_hash` 的多 source 观测计数；主线产品 smoke 已覆盖未授权 source policy 写入 fail-closed、source quorum 不足 fail-closed 与第二个许可 source peer 观测后通过。该 source policy 已支持 `disabled_peer_ids`、`disabled_peer_reasons/slashing_reasons` 和 `peer_rotations`，被禁用 source peer 不得作为证明来源且不计入 quorum，old peer -> new peer 的 rotation 可审计；主线治理 `nov_setMappedHeaderAttestationPolicy` 已可约束 Ed25519 header attestation public key、`min_attestation_quorum`、`disabled_signers`、`disabled_signer_reasons` 和 `signer_rotations`，live proof 必须携带对 `chain_id/block_number/block_hash/receipts_root` 的 `header_attestations` 签名，主线产品 smoke 已覆盖未授权 attestation policy 写入 fail-closed、attestation quorum 不足/disabled signer fail-closed 与两个 active signer 通过；`mapped_lock_min_confirmations` 已可由 governance/Treasury policy 设置；`nov_getMappedFinalitySourceStatus` 只读聚合当前 lock contract/finality source 状态；`nov_runMappedAssetAutoHeal` 提供治理开启后的显式 unsafe asset auto-heal tick，并可在独立 rollback policy 开启后自动终止 unsafe frozen asset，但还没有完整 finality source 管理、常驻后台调度、治理赔付或链上出金。
...
- `ua_autoHealMappedAssets` 提供最小自动 reorg heal 执行能力；`nov_runMappedAssetAutoHeal` 是主线显式 scheduler tick：dry-run 报告候选、所需 policy 和当前 policy 是否会执行；`apply=true` 必须带 `scheduler_authorized=true`。`mapped_asset_reorg_response_policy=report_only` 只报告，`freeze_only` 冻结 unsafe active/burn_pending live NETH，`freeze_and_rollback` 额外允许终止 unsafe frozen NETH。无论 freeze 还是 rollback，都不触发外部链释放、NOV mint 或赔付。
- 仍没有完整 external finality source 管理、治理赔付或链上出金；当前只具备 mainline governance wrapper 管理的 source peer whitelist/quorum、disabled peer slashing reason/fail-closed、source peer rotation 记录、Ed25519 attestation quorum、disabled signer reason/fail-closed、signer rotation 记录、最小 confirmations 和只读 finality status 聚合；未授权 policy 写入 fail-closed、source quorum fail-closed/二源通过、attestation quorum fail-closed/双 active signer 通过和 governed min confirmations fail-closed/放宽后通过已有主线产品 smoke。
...
- MVP live 模式已有 receipt MPT inclusion + 本地 canonical finalized block anchor + Ethereum lock event evidence -> NETH/M2 credit 的内部账本接线，并已有 mapped bridge pause 门禁、source anchor reorg gate、按同一 `block_hash` 多 source 观测计数的 header source whitelist/quorum gate、source quorum fail-closed/二源通过主线 smoke、disabled source peer slashing reason/fail-closed、source peer rotation 记录、Ed25519 header attestation signature quorum gate、attestation quorum fail-closed/双 active signer 通过主线 smoke、disabled signer reason/fail-closed、signer rotation 记录、governed min confirmations fail-closed/放宽后通过主线 smoke、`nov_getMappedFinalitySourceStatus` 只读状态聚合、manual freeze/recovery/rollback，以及 governance/Treasury policy 分别控制的 auto-freeze 和 unsafe frozen auto-rollback heal；但没有完整 finality source 管理、治理赔付或链上自动出入金。
...
- mapped lock proof live 模式目前已验证 receipt MPT inclusion、receipt status、receipt log address/topic0、结构化 event evidence、本地 runtime canonical finalized block anchor，并可在治理开启后验证 header source peer 白名单、`min_source_quorum`、Ed25519 header attestation public key 白名单、`min_attestation_quorum`、disabled signer、禁用原因和 signer rotation 记录。
...
- live proof 已固定治理 lock contract 配置、事件 topic、source_chain_id、block_hash、receipt_index、receipt_log_index、receipt MPT proof、finalized block number、本地 finalized canonical block anchor 和 `source_lock_ref` 派生校验；治理 lock contract 优先级高于参数级 expected address。
- `mapped_lock_min_confirmations` 已可由 governance/Treasury policy 设置，live ETH lock proof 优先用 native store policy 校验 finalized depth；主线产品 smoke 已覆盖未达标 fail-closed 与放宽后通过。
- `nov_setMappedHeaderSourcePolicy` / `nov_setMappedHeaderAttestationPolicy` 已可通过 mainline governance wrapper 设置治理许可 source peer、Ed25519 header attestation public key 集合、disabled peer/signer 集合、禁用原因、rotation 和 quorum；未带 `governance_authorized=true` 的写入会 fail-closed。live ETH lock proof 携带的 `header_attestations` 必须对 `chain_id/block_number/block_hash/receipts_root` 签名，签名无效、signer 被禁用或达不到 quorum 时 fail-closed。
...
- `ua_autoHealMappedAssets` 已能扫描 unsafe source anchor；主线显式 tick `nov_runMappedAssetAutoHeal` 已能在 dry-run 下报告候选，并在 `scheduler_authorized=true` 且 governance/Treasury policy 通过 `nov_applyTreasuryPolicy` 开启 `mapped_asset_auto_heal_enabled` 后用 `apply=true` 自动冻结 active/burn_pending live mapped asset；对已 frozen 且 source anchor 仍 unsafe 的 asset，只有额外开启 `mapped_asset_auto_heal_rollback_enabled` 才会自动 rollback。主线产品 smoke 已覆盖 dry-run、未授权 apply fail-closed、mainline policy freeze apply 和 mainline policy rollback apply。默认 dry-run 不改状态，未开启基础 auto-heal 或未授权 scheduler 时 fail-closed；常驻后台 scheduler 仍未完成。
- `nov_getM2BridgeRiskStatus.operations_readiness` 已把 P0 桥接声明边界机器可读化：`external_bridge_complete=false`、`external_release_ready=false`、`background_scheduler_ready=false`、`compensation_policy_ready=false`、`nov_emission_policy_ready=false`，并列出进入 external bridge v1 前仍需 `deployed_governed_lock_contract / external_finality_source_management / automated_reserve_proof_verification / chain_release_transaction_path / governance_compensation_policy / background_scheduler_or_operator_runbook / nov_emission_policy_if_any`。
- `TokenRuntime::mint` 没有接到 Treasury policy / emission policy 路径。

建议：

- 下一步只做最小产品桥：
  - 把当前 header source whitelist/quorum gate、Ed25519 header attestation quorum gate、disabled signer gate、disabled reason 和 signer rotation 记录继续升级为完整 external finality source 管理。
  - 固定 lock contract address 配置已进入 live proof gate，并已可通过 `nov_applyTreasuryPolicy(mapped_lock_contract_address)` 从 mainline governance 写入 native store；后续仍需要真实部署合约地址、治理变更流程和外部链监听接线。
  - 当前已验 receipt MPT inclusion、`Locked(address indexed owner, bytes32 lockId, uint256 amount, string targetUca)` 的 receipt log address/topic0、本地 finalized canonical block anchor、可选治理 header source whitelist/quorum、disabled source peer slashing reason、source peer rotation、可选治理 Ed25519 header attestation signature quorum、disabled signer、禁用原因和 signer rotation；source quorum 已按同一 `block_hash` 多 source 观测计数，未授权 finality policy 写入 fail-closed、source quorum fail-closed/二源通过已有主线 smoke，attestation quorum fail-closed/双 active signer 通过已有主线 smoke，最小 finalized confirmations 已可治理设置且已有主线 smoke 覆盖 fail-closed/放宽后通过，reorg heal 主线入口 smoke 已覆盖 explicit tick dry-run、未授权 apply fail-closed、通过 `nov_applyTreasuryPolicy` 启用 freeze/rollback 后执行。下一步补完整 finality source 管理、治理赔付和常驻后台调度。
  - `receipts_root` 已不再只信任用户自报；finalized block number 仍只作为确认数约束字段，最终锚定以本地 canonical finalized block 为准。
  - bridge pause 已能阻断 live register/burn/release；source anchor reorg gate 已能阻断 unsafe burn/release；header source whitelist/quorum gate 已能阻断非许可 source peer、被禁用 source peer 或 quorum 不足；Ed25519 header attestation quorum gate 已能阻断无效签名、禁用 signer 或许可 signer quorum 不足；manual freeze/unfreeze/rollback 已能冻结、安全恢复或终止异常 NETH 暴露；`nov_runMappedAssetAutoHeal` 已能在 `nov_applyTreasuryPolicy` 治理开启后执行 scheduler-authorized auto-heal tick，自动冻结 unsafe active/burn_pending live NETH，并在额外开启 rollback policy 后自动终止 unsafe frozen NETH；下一步补完整 finality source、治理赔付规则和常驻后台调度。
...
- 许可 oracle 只能作为治理白名单参考源，不能开放喂价；`oracle_open_feed_allowed` 必须保持 false；治理禁用的 source 即使仍在 allowlist 中也必须 fail-closed，不得参与 `P_ref`。
- 低流动性、oracle 偏离、储备 proof 非 active、active proof amount cap 不足、储备不足时必须进入 constrained/blocked、fail-closed 或只允许 NOV 支付。
