...
- 仍没有真实“完整 external finality source 管理 -> compensation -> Treasury policy -> NOV emission”的完整链上桥接。治理化 header source peer quorum、disabled peer slashing reason/fail-closed、source peer rotation 记录、Ed25519 header attestation signature quorum、disabled signer reason/fail-closed、signer rotation 记录和 `ua_getMappedFinalitySourceStatus` 只读状态聚合已有；主线产品 smoke 已覆盖 source quorum 不足 fail-closed 与第二个许可 source peer 观测后通过、attestation quorum 不足/disabled signer fail-closed 与两个 active signer 通过、governed min confirmations 未达标 fail-closed 与降低确认数后通过。但 NOV mint 在 consensus token runtime 中存在，不能直接接在 ETH lock proof 上。
...
- 这不是完整真实外部桥；live 模式的 receipt MPT proof 已要求锚定本地 runtime canonical finalized block，并可通过 `ua_setMappedHeaderSourcePolicy` 约束 header source peer 和 `min_source_quorum`，且 quorum 已按同一 `block_hash` 的多 source 观测计数；主线产品 smoke 已覆盖 source quorum 不足 fail-closed 与第二个许可 source peer 观测后通过。该 source policy 已支持 `disabled_peer_ids`、`disabled_peer_reasons/slashing_reasons` 和 `peer_rotations`，被禁用 source peer 不得作为证明来源且不计入 quorum，old peer -> new peer 的 rotation 可审计；`ua_setMappedHeaderAttestationPolicy` 已可约束 Ed25519 header attestation public key、`min_attestation_quorum`、`disabled_signers`、`disabled_signer_reasons` 和 `signer_rotations`，live proof 必须携带对 `chain_id/block_number/block_hash/receipts_root` 的 `header_attestations` 签名，主线产品 smoke 已覆盖 attestation quorum 不足/disabled signer fail-closed 与两个 active signer 通过；`mapped_lock_min_confirmations` 已可由 governance/Treasury policy 设置；`nov_runMappedAssetAutoHeal` 提供治理开启后的显式 unsafe asset auto-heal tick，并可在独立 rollback policy 开启后自动终止 unsafe frozen asset，但还没有完整 finality source 管理、常驻后台调度、治理赔付或链上出金。
...
- 仍没有完整 external finality source 管理、治理赔付或链上出金；当前只具备 source peer whitelist/quorum、disabled peer slashing reason/fail-closed、source peer rotation 记录、Ed25519 attestation quorum、disabled signer reason/fail-closed、signer rotation 记录、最小 confirmations 和只读 finality status 聚合；source quorum fail-closed/二源通过、attestation quorum fail-closed/双 active signer 通过和 governed min confirmations fail-closed/放宽后通过已有主线产品 smoke。
...
- `ua_setMappedHeaderAttestationPolicy` 已可设置治理许可 Ed25519 header attestation public key 集合、disabled signer 集合、disabled signer reason、signer rotation 和 quorum；live ETH lock proof 携带的 `header_attestations` 必须对 `chain_id/block_number/block_hash/receipts_root` 签名，签名无效、signer 被禁用或达不到 quorum 时 fail-closed。
...
- 下一步只做最小产品桥：
  ...
  - 当前已验 receipt MPT inclusion、`Locked(address indexed owner, bytes32 lockId, uint256 amount, string targetUca)` 的 receipt log address/topic0、本地 finalized canonical block anchor、可选治理 header source whitelist/quorum、disabled source peer slashing reason、source peer rotation、可选治理 Ed25519 header attestation signature quorum、disabled signer、禁用原因和 signer rotation；source quorum 已按同一 `block_hash` 多 source 观测计数，source quorum fail-closed/二源通过已有主线 smoke，attestation quorum fail-closed/双 active signer 通过已有主线 smoke，最小 finalized confirmations 已可治理设置且已有主线 smoke 覆盖 fail-closed/放宽后通过，reorg heal 主线入口 smoke 已覆盖 explicit tick dry-run、未授权 apply fail-closed、通过 `nov_applyTreasuryPolicy` 启用 freeze/rollback 后执行。下一步补完整 finality source 管理、治理赔付和常驻后台调度。
