...
- M2 auto heal v1 已接入 `ua_autoHealMappedAssets`：默认 dry-run 报告 unsafe source anchor，并对每个候选返回 `required_policy/policy_would_apply`；governance/Treasury policy 可用 `mapped_asset_reorg_response_policy` 统一配置为 `report_only / freeze_only / freeze_and_rollback`。`freeze_only` 可自动冻结 active/burn_pending live NETH，扣减用户 native 可用余额并保留 Treasury reserve；`freeze_and_rollback` 额外允许把 unsafe frozen NETH 内部回滚为 `rejected` 并扣回 Treasury reserve。主线产品 smoke 已覆盖 live NETH 入账后 source anchor reorg -> dry-run -> policy freeze -> policy rollback。
...
- `ua_autoHealMappedAssets` 已能扫描 unsafe source anchor，并在 governance/Treasury policy 开启 `mapped_asset_auto_heal_enabled` 后用 `apply=true` 自动冻结 active/burn_pending live mapped asset；对已 frozen 且 source anchor 仍 unsafe 的 asset，只有额外开启 `mapped_asset_auto_heal_rollback_enabled` 才会自动 rollback。主线产品 smoke 已覆盖 dry-run、freeze apply 和 rollback apply。默认 dry-run 不改状态，未开启基础 auto-heal 时 fail-closed。
...
- 下一步只做最小产品桥：
  ...
  - 当前已验 receipt MPT inclusion、`Locked(address indexed owner, bytes32 lockId, uint256 amount, string targetUca)` 的 receipt log address/topic0、本地 finalized canonical block anchor、可选治理 header source whitelist/quorum、disabled source peer slashing reason、source peer rotation、可选治理 Ed25519 header attestation signature quorum、disabled signer、禁用原因和 signer rotation；source quorum 已按同一 `block_hash` 多 source 观测计数，source quorum fail-closed/二源通过已有主线 smoke，attestation quorum fail-closed/双 active signer 通过已有主线 smoke，最小 finalized confirmations 已可治理设置且已有主线 smoke 覆盖 fail-closed/放宽后通过，reorg heal 主线入口 smoke 已覆盖 dry-run/freeze/rollback。下一步补完整 finality source 管理、治理赔付和自动化调度。
  - `receipts_root` 已不再只信任用户自报；finalized block number 仍只作为确认数约束字段，最终锚定以本地 canonical finalized block 为准。
  - bridge pause 已能阻断 live register/burn/release；source anchor reorg gate 已能阻断 unsafe burn/release；header source whitelist/quorum gate 已能阻断非许可 source peer、被禁用 source peer 或 quorum 不足；Ed25519 header attestation quorum gate 已能阻断无效签名、禁用 signer 或许可 signer quorum 不足；manual freeze/unfreeze/rollback 已能冻结、安全恢复或终止异常 NETH 暴露；`ua_autoHealMappedAssets` 已能在治理开启后自动冻结 unsafe active/burn_pending live NETH，并在额外开启 rollback policy 后自动终止 unsafe frozen NETH；下一步补完整 finality source、治理赔付规则和自动化调度。
