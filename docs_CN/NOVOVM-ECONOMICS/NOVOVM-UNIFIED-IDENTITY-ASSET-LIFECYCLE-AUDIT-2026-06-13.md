...
- 授权披露 receipt v1 已落代码：授权的用户级 M2 查询会返回 `privacy_disclosure`，用于证明本次明细披露经过 mainline read gate，并用 `result_commitment_digest` 绑定本次授权响应；`nov_verifyPrivacyDisclosureReceipt` 可验证原响应通过、篡改响应失败；该 receipt 当前是审计票据，不是 RingCT/ZK proof。
