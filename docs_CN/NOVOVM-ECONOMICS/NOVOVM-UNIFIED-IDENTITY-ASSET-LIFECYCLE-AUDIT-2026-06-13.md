...
- 授权披露 receipt v1 已接入主线资产查询：`nov_getAssetBalance`、`account_balance`、`account_assets` 在合法授权返回用户级 M2 明细时附带 `privacy_disclosure`，记录授权原因、truth source、`proof_generation_performed=false`、可升级 proof path（RingCT/ZK）和 disclosure digest；未授权路径仍只返回 redacted，不返回余额或资产列表。
...
- 授权披露 receipt v1 已落代码：授权的用户级 M2 查询会返回 `privacy_disclosure`，用于证明本次明细披露经过 mainline read gate；该 receipt 当前是审计票据，不是 RingCT/ZK proof。
