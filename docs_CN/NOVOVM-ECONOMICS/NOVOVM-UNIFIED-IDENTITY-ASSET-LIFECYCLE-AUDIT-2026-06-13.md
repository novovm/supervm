...
- M2 bridge risk status v1 已接入主线只读入口 `nov_getM2BridgeRiskStatus`：聚合 `ua_getMappedFinalitySourceStatus`、Treasury `get_reserve_proof`、native mapped lock / burn / release / auto-heal policy，返回 `active / constrained / blocked` 风险口径；缺失或失效 reserve proof、缺失 governed lock contract、缺失 governed min confirmations、bridge/burn/release pause 都按 `blocked` fail-closed；finality source 处于 `open_unprotected` 时按 `constrained` 返回，并显式透出 `finality_gap=*` 原因；该入口只读，不创建账户、不写 UCA、不触发外部释放、不 mint NOV、不启动后台调度。
