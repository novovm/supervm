...
隐私边界：协议清算价、Treasury reserve、M2 liability、reserve proof 和风险状态属于系统级公开数据；用户级 nAsset 余额、交易流水、KYC 身份和外部地址绑定默认不公开。当前最小产品边界由 mainline read gate 返回 `privacy_redacted=true`；使用 `N*` M2 资产支付 Execution Fee 的用户执行入口会自动要求 `privacy_required`，public 路径 fail-closed。AOEM RingCT / ZK 是后续密码学隐私执行/证明层，不能绕过 mainline unified account surface，也不能产生第二套 nAsset 账本。主线只读入口 `nov_getPrivacyCapabilityStatus` 已用于暴露 read gate / RingCT / ZK / MLDSA 分层状态；它不生成 proof、不暴露用户余额，且明确 `MLDSA` 只是 `pq_signature_only` 抗量子签名能力。主线只读入口 `nov_getPrivacyAuditPolicy` 固化受保护用户级 M2 查询面为 `nov_getAssetBalance`、`account_balance`、`account_assets`，默认遮蔽，授权后必须携带 `privacy_disclosure`，且必须可由 `nov_verifyPrivacyDisclosureReceipt` 验证。授权的用户级 M2 查询会返回 `privacy_disclosure` 审计票据，记录授权原因、truth source、`result_commitment_digest` 和 disclosure digest；该票据不是 RingCT/ZK proof。
...
- M2 bridge operations readiness v1 已接入 `nov_getM2BridgeRiskStatus.operations_readiness`：`risk_gate_state=active` 只表示 internal M2 credit / risk-gated Treasury settlement 可用；`external_bridge_complete`、`external_release_ready`、`background_scheduler_ready`、`compensation_policy_ready`、`nov_emission_policy_ready` 固定为 `false`，直到真实 external finality source 管理、自动 reserve proof verification、链上 release transaction、治理赔付策略、scheduler/runbook 和 NOV emission policy 明确接入。
- 隐私能力状态查询 v1 已接入主线 native query：`nov_getPrivacyCapabilityStatus` 返回 `read_gate_only / privacy_proof_capable`、RingCT/ZK prove/verify capability、MLDSA PQ signature-only role 和 M2 read redaction policy；该入口只读，不生成隐私 proof，不泄露用户级 nAsset 余额。
- 隐私审计策略查询 v1 已接入主线 native query：`nov_getPrivacyAuditPolicy` 返回受保护用户级 M2 查询方法、默认 redaction、授权披露 receipt 要求、验证方法、gateway bypass 禁止、no second ledger 和 RingCT/ZK proof upgrade path；该入口只读，不生成 proof，不写账本。
- 授权披露 receipt v1 已接入 `nov_getAssetBalance`、`account_balance` 和 `account_assets`：授权时返回 `privacy_disclosure`，并用 `result_commitment_digest` 绑定本次授权响应；`nov_verifyPrivacyDisclosureReceipt` 可只读验证三类受保护查询的匹配关系，并在响应被篡改时失败；未授权仍 redacted；该路径是 read gate 的审计输出，不是完整密码学隐私 proof。
