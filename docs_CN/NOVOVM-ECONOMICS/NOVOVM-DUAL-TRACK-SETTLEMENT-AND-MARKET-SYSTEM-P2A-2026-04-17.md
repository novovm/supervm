...
- 只读查询 `nov_call treasury.get_protocol_clearing_price` 与 mainline wrapper `nov_getProtocolClearingPrice` 可返回 `P_epoch/P_pay/P_redeem`、source、rejected source、epoch、attack-resistance 状态和 oracle source allowlist/disabled/rotation 状态；非白名单、已禁用或过期 oracle source 会被记录为 rejected source，不进入 `P_ref`，也不得作为无快照时的 `P_prev` 来源。主线产品 smoke 已覆盖 disabled/stale oracle 被拒绝、`epoch_fixed=true`、`amm_spot_allowed=false`、`P_pay < P_epoch < P_redeem`。
