...
- `ua_registerMappedLock(phase4_mode=live)` 已要求结构化 Ethereum lock event evidence 和 receipt MPT proof，并能把通过校验的 ETH lock MVP 映射为 `NETH` M2 credit，写入 native account balance、Treasury reserve 和 settlement journal；`ua_burnMappedAsset -> ua_releaseMappedLock` 已能扣减用户 NETH credit 并释放 Treasury NETH reserve。
- 当前仍不声明可信 Ethereum block header/finality source、reorg heal、真实链上出金、跨链自动 mint/redeem、NOV 直接铸造或钱包/DAPP/网站完成。
