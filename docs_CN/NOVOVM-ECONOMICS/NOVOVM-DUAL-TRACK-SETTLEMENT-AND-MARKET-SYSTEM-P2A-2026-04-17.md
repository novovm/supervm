...
- 授权披露 receipt v1 已接入 `nov_getAssetBalance`、`account_balance` 和 `account_assets`：授权时返回 `privacy_disclosure`，未授权仍 redacted；该路径是 read gate 的审计输出，不是完整密码学隐私 proof。
