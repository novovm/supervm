...
- M2 auto heal v1 已接入 `ua_autoHealMappedAssets`：默认 dry-run 报告 unsafe anchor，并对每个候选返回 `required_policy/policy_would_apply`；governance/Treasury policy 可通过 `nov_applyTreasuryPolicy(mapped_asset_reorg_response_policy)` 统一配置为 `report_only / freeze_only / freeze_and_rollback`。`freeze_only` 可自动冻结 active/burn_pending live NETH，扣用户 native 可用余额、保留 Treasury reserve；`freeze_and_rollback` 额外允许对已 frozen 且 source anchor 仍 unsafe 的资产自动执行内部 rollback，扣回 Treasury NETH reserve 并把 mapped asset 置为 `rejected`。主线产品 smoke 已覆盖 live NETH 入账后 source anchor reorg -> dry-run -> mainline policy freeze -> policy rollback；该路径不赔付、不链上出金、不 mint NOV，也不等于后台 scheduler 已完成。
