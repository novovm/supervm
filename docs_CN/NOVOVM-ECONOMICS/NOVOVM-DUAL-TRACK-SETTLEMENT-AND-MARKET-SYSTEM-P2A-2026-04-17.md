...
- M2 manual freeze/recovery v1 已接入 `ua_freezeMappedAsset` / `ua_unfreezeMappedAsset`：active live NETH 冻结会扣用户 native 可用余额、保留 Treasury reserve，并把 mapped asset 状态置为 `frozen`；source anchor 恢复 canonical finalized 后才能 unfreeze，恢复用户 native 可用余额。
