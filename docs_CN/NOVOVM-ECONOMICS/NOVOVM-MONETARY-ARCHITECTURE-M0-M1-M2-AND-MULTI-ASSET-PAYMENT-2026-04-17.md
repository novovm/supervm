...
8. M2 auto heal v1 已落代码：`ua_autoHealMappedAssets` 默认 dry-run，只报告 unsafe source anchor 和每个候选的 `required_policy/policy_would_apply`；governance/Treasury policy 可通过主线 `nov_applyTreasuryPolicy` 的 `mapped_asset_reorg_response_policy` 统一配置为 `report_only / freeze_only / freeze_and_rollback`。`apply=true` 必须先开启对应策略，否则 fail-closed 或只报告。`freeze_only` 可自动冻结 active/burn_pending live NETH，扣减用户 native 可用余额并保留 Treasury reserve；`freeze_and_rollback` 额外允许对已 frozen 且 source anchor 仍 unsafe 的资产执行内部 rollback：扣回 Treasury NETH reserve、把 mapped asset 置为 `rejected`。主线产品 smoke 已覆盖 live NETH 入账后 source anchor reorg -> dry-run -> `nov_applyTreasuryPolicy(freeze_only)` -> freeze apply -> `nov_applyTreasuryPolicy(freeze_and_rollback)` -> rollback apply；它不赔付、不链上出金、不 mint NOV。
...
2. 把 `ua_autoHealMappedAssets` 接到主线调度：当前主线产品入口、dry-run、通过 `nov_applyTreasuryPolicy` 启用的 freeze apply 和 frozen unsafe rollback apply 已覆盖；仍需要后台调度触发、治理赔付规则和完整外部 finality source 管理。
