...
9. 隐私能力状态查询 v1 已落代码：`nov_getPrivacyCapabilityStatus` 只读暴露 mainline read gate、AOEM RingCT、AOEM ZK 和 MLDSA 的分层能力状态；该入口不生成 proof、不查询或泄露用户余额、不创建第二账本。`MLDSA` 固定标记为 `pq_signature_only`，只属于账户/交易签名抗量子能力，不等同隐私交易系统。
10. 授权披露 receipt v1 已落代码：`nov_getAssetBalance`、`account_balance`、`account_assets` 在合法授权返回用户级 M2 明细时附带 `privacy_disclosure`，记录授权原因、truth source、`proof_generation_performed=false`、可升级 proof path（RingCT/ZK）、`result_commitment_digest` 和 deterministic disclosure digest；`nov_verifyPrivacyDisclosureReceipt` 可只读验证披露票据与授权响应是否匹配；未授权路径仍只返回 redacted，不返回余额或资产列表。
...
14. 隐私能力状态查询 v1 已落代码：`nov_getPrivacyCapabilityStatus` 从参数或 `NOVOVM_AOEM_CAPABILITIES_JSON` 读取 AOEM capability contract，返回 `read_gate_only` 或 `privacy_proof_capable`，并明确 RingCT/ZK/MLDSA 的职责分层；该入口只读、不生成 proof、不暴露用户级 M2 余额。
15. 隐私审计策略查询 v1 已落代码：`nov_getPrivacyAuditPolicy` 固化受保护用户级 M2 查询面为 `nov_getAssetBalance`、`account_balance`、`account_assets`，默认 `redacted`，授权披露必须带 `privacy_disclosure`，并要求用 `nov_verifyPrivacyDisclosureReceipt` 验证 `result_commitment_digest`。该入口只读、无 gateway bypass、无第二账本，不生成 RingCT/ZK proof。
16. 授权披露 receipt v1 已落代码：授权的用户级 M2 查询返回 `privacy_disclosure`，把 read gate 的授权理由、响应结果承诺 `result_commitment_digest` 和 disclosure digest 显式化；`nov_verifyPrivacyDisclosureReceipt` 已覆盖 `nov_getAssetBalance`、`account_balance`、`account_assets` 三类受保护查询，可检测响应被篡改或 digest 不匹配；该 receipt 是审计票据，不是 RingCT/ZK proof，也不是 encrypted balance 账本。
17. Native execution store 已具备最小 lockfile single-writer guard：主写路径先获取写锁，再执行 load/modify/save，适合单机/低并发产品闭环；该 guard 不是 RocksDB/事务后端，也不代表高并发账本入口完成。
18. 当前仍不声明真实外部桥、完整 external finality source 管理、治理赔付、真实链上出金、完整自动 reserve proof verification、完整桥接铸造/赎回自动化、完整 encrypted balance/ZK proof 或多进程高并发账本入口完成；当前 finality source 管理只覆盖 source peer whitelist/quorum、disabled peer slashing reason/fail-closed、source peer rotation 记录、Ed25519 attestation quorum、disabled signer reason/fail-closed、signer rotation 记录、最小 confirmations 和 `nov_getMappedFinalitySourceStatus` 只读状态聚合。主线产品 smoke 已覆盖未授权 finality policy 写入 fail-closed、source quorum 不足 fail-closed 与第二个许可 source peer 观测后通过、attestation quorum 不足/disabled signer fail-closed 与两个 active signer 通过、governed min confirmations 未达标 fail-closed 与降低确认数后通过。治理层 oracle source allowlist / disabled reason / rotation mainline write、reserve proof mainline write、mainline protocol clearing price query、`P_redeem` reserve redeem、reserve proof effective-status gate、`nov_getM2BridgeRiskStatus.operations_readiness` 外部桥非声明边界、nAsset 最小 read gate、N* fee asset privacy-required gate、`nov_getPrivacyCapabilityStatus` 能力状态查询、`nov_getPrivacyAuditPolicy` 隐私审计策略和授权披露 receipt 已具备最小执行约束，但不是开放喂价系统，也不等于完整 oracle 网络、自动 reserve verification、完整外部桥或完整密码学隐私账本。
