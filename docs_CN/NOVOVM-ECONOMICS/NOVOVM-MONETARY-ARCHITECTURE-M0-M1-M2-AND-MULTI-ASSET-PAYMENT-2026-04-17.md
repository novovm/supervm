...
10. 授权披露 receipt v1 已落代码：`nov_getAssetBalance`、`account_balance`、`account_assets` 在合法授权返回用户级 M2 明细时附带 `privacy_disclosure`，记录授权原因、truth source、`proof_generation_performed=false`、可升级 proof path（RingCT/ZK）、`result_commitment_digest` 和 deterministic disclosure digest；未授权路径仍只返回 redacted，不返回余额或资产列表。
...
15. 授权披露 receipt v1 已落代码：授权的用户级 M2 查询返回 `privacy_disclosure`，把 read gate 的授权理由、响应结果承诺 `result_commitment_digest` 和 disclosure digest 显式化；该 receipt 是审计票据，不是 RingCT/ZK proof，也不是 encrypted balance 账本。
