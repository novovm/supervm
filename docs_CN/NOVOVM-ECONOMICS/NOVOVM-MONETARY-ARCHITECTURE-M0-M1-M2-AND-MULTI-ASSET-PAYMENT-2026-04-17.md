...
10. M2 finality policy v1 已落代码：`mapped_lock_min_confirmations` 可由 governance/Treasury policy 设置；live ETH lock proof 会优先使用 native store policy，未设置时才 fallback 到 env/default。它只管理最小 finalized confirmations，不等于完整 finality source 管理。
...
13. 当前仍不声明真实外部桥、完整 external finality source 管理、治理赔付、真实链上出金、完整自动 reserve proof verification、完整桥接铸造/赎回自动化或多进程高并发账本入口完成；当前 finality source 管理只覆盖 source peer whitelist/quorum、disabled peer slashing reason/fail-closed、source peer rotation 记录、Ed25519 attestation quorum、disabled signer reason/fail-closed、signer rotation 记录、最小 confirmations 和 `ua_getMappedFinalitySourceStatus` 只读状态聚合。主线产品 smoke 已覆盖 source quorum 不足 fail-closed 与第二个许可 source peer 观测后通过。治理层 oracle source allowlist / disabled reason / rotation 与 reserve proof effective-status gate 已具备最小执行约束，但不是开放喂价系统，也不等于完整 oracle 网络或自动 reserve verification。
