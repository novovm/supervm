...
10. 当前仍不声明真实外部桥、完整 external finality source 管理、治理赔付、真实链上出金、治理层 oracle 白名单管理、完整桥接铸造/赎回自动化或多进程高并发账本入口完成；当前 finality source 管理只覆盖 source peer quorum、Ed25519 attestation quorum、disabled signer reason/fail-closed 和最小 confirmations。
...
1. 把当前治理化 header source whitelist/quorum gate 和 Ed25519 header attestation quorum gate 继续升级到完整 external finality source 管理，包括 signer key rotation、source slashing action、reorg response policy；`receipts_root` 已不再只信任用户输入，quorum 已按同一 `block_hash` 的多 source 观测计数，最小 finalized confirmations 已可由 governance/Treasury policy 设置，attestation signature quorum、disabled signer 和 disabled reason 已可由 mainline UCA policy 设置。
