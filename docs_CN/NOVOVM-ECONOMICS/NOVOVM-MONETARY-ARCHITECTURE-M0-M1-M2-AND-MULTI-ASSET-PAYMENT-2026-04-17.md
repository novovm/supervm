...
10. Treasury reserve proof v1 已具备最小治理登记/查询与执行门禁面：`governance.set_reserve_proof` 可按资产登记 proof type/digest/source/reference/amount/status；`treasury.get_reserve_proof` 和 `treasury.get_reserve_snapshot` 可只读暴露 effective status 与 non-claim 标记。若某资产已登记 proof 且 effective status 非 `active`，该资产的非 NOV fee clearing 和 treasury redeem 会 fail-closed；若 active proof 的 `reserve_amount` 低于 projected Treasury reserve/exposure，非 NOV fee clearing 会拒绝扩张，treasury redeem 也必须在操作后不继续高于 proof cap。该路径仍不做自动外部验真、不授权 NOV mint、不授权外部赎回。
