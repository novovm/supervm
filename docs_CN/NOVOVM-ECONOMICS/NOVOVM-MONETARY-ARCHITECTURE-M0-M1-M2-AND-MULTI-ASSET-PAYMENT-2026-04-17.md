...
16. 授权披露 receipt v1 已落代码：授权的用户级 M2 查询返回 `privacy_disclosure`，把 read gate 的授权理由、响应结果承诺 `result_commitment_digest` 和 disclosure digest 显式化；`nov_verifyPrivacyDisclosureReceipt` 可检测响应被篡改或 digest 不匹配；该 receipt 是审计票据，不是 RingCT/ZK proof，也不是 encrypted balance 账本。
