...
- EVM gateway Engine API probe 已接入：`engine_exchangeCapabilities` 返回当前真实支持的 `["engine_exchangeCapabilities"]`，其它 `engine_*` payload/forkchoice 控制面仍禁用，避免在没有真实 CL forkchoice/newPayload 语义前对外伪造 geth Engine API 等价。
