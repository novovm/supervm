...
- 本轮真实主网产品入口 follow-up 从 `current=1077/highest=25274898` 推进到 `current=1181/highest=25275112`；期间 block `1093`/`1149` 出现 mid-body remote close 后均由 missing-body recovery 恢复。`1149` 暴露了 retained historical body gaps 会在 `highest > current` 时抢占 header pull 的追高阻塞；现在追高状态只恢复 current/latest missing body，旧缺口不再阻塞 forward headers，修正后从 `1149` 继续推进到 `1181`，block `1181` 保留了非空 ommer body material 和 receipt snapshot。
