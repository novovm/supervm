...
- `BlockHeaders` 批次导入前会先校验相邻 header 的 `number` 连续且子 header `parentHash` 等于上一 header hash；编号跳跃或拼接批次按 peer decode failure 拒绝，不写入 native header/materialization 队列。
