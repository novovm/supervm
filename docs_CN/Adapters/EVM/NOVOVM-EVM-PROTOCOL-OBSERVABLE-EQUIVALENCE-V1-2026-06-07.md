...
- Stalled peer refresh 已接入 `novovm-node` 产品入口：当公共 peer churn 导致未全 cooldown 但 `highest > current` 且连续无 ready/无同步推进时，会按 `NOVOVM_ETH_RLPX_STALLED_REFRESH_INTERVAL_TICKS` 扩容或刷新候选；初始候选上限放开到 512，自适应上限放开到 1024，活跃连接仍由 `NOVOVM_ETH_RLPX_MAX_PEERS` 控制；本轮 40 tick probe 在 `current=1024/highest=25268829` 后触发 `reason=no_ready_sync_progress_stalled_expand`，候选从 103 刷新到 113。对照本地最新 geth `ProtocolVersions = ETH71, ETH70, ETH69`，eth/68-only peer 仍按不兼容处理，不为追求连接率而偏离当前 geth 产品面。
