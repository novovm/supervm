...
- `novovm-node` 已新增直接产品入口 `NOVOVM_NODE_MODE=eth_rlpx_sync`，不经过临时脚本即可启动 native Ethereum RLPx worker；本次真实主网有限 tick 验证中，节点完成 Status、Headers、Bodies、Receipts，native current 从 0 推进到 2048。当前仍依赖静态 enode，不等于 geth 的 discovery/peer replacement/长期追高。
