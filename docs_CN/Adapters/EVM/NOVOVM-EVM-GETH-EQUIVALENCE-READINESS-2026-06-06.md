...
- novovm-node RLPx native head store gate: pass, `NOVOVM_ETH_RLPX_NATIVE_HEAD_STORE_ENABLED` 默认开启，`NOVOVM_ETH_RLPX_NATIVE_HEAD_STORE_PATH` 可覆盖路径；产品入口会持久化最新已校验 native header/body/receipt 快照，启动时恢复到 runtime head，receipt 可用时恢复 phase=`State` 而不是退回 `Headers`，产品入口临时 store 恢复验证输出 `current=77 highest=99 header_number=77 body_available=true`；still does not claim full historical block/state/receipt database
