...
- RLPx Receipts gate: pass, covers real `BlockHeaders`/`BlockBodies` sync -> follow-up eth/70 `GetReceipts(firstBlockReceiptIndex=0)` -> `Receipts(lastBlockIncomplete=false)` parsed before peer sync ready
...
- RLPx NewBlock gate: pass, covers inbound real non-empty `NewBlock` announcement -> Ethereum transaction trie `transactionsRoot` validation -> empty ommers/withdrawals validation -> native header/body snapshot import -> peer head/highest update -> follow-up `GetReceipts`
