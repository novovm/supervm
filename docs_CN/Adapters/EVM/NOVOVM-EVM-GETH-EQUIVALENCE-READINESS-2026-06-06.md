...
- Ethereum discv4 discovery gate: pass, covers signed discv4 Ping/Pong/random-target FindNode/Neighbors packet build/parse, bootnode endpoint proof bonding, inbound Ping -> Pong reply, public IPv4 Neighbors -> `enode://` candidate materialization, and mixed IPv4/IPv6 Neighbors parsing with unsupported IPv6 skipped rather than failing the whole packet；discovery-only live run with 4 geth mainnet bootnodes returned `endpoints=9` with `neighbor_parse_errors=0`；random-target follow-up live run returned `endpoints=12` from a single bootnode；discv4+DNS 16 tick live run returned `discv4 endpoints=29` plus `DNS endpoints=15`, then RLPx sync advanced to `current=1024/highest=25267957`；still does not claim full discv4 Kademlia table/random walk, discv5, or long-haul peer churn
