...
runtime reputation reorder 生效后，16 tick 仍可能在当前公网 peer 集合中没有 ready peer；固定 fanout=8 意味着 16 tick 只实际尝试约 128 个 bootstrap endpoint。历史阶段曾让 admission stalled 时的 runtime fanout 自动提升到 active window 上限，以更快消耗坏候选；2026-06-10 后默认产品入口重新收敛为 fanout=8，只有显式设置 `NOVOVM_ETH_RLPX_SYNC_TARGET_FANOUT` / `NOVOVM_ETH_RLPX_ADAPTIVE_BOOTSTRAP_FANOUT` 才执行高 fanout 探测。
...
27. 2026-06-09 最新官方 geth `debug_clearTxpool` 复核：`D:\WEB3_AI\go-ethereum` 已同步到 `origin/master = 08aaa7c5ff9dcb99fb0c6dc9194d49ae9e518f69`，相对上一轮 `10614fc42` 只有 `eth: add debug_clearTxpool api (#33347)`。该提交只在 geth debug API 增加清空本地 txpool 的 RPC，并给 web3 extension 增加入口；它不改变 RLPx、snap、headers/bodies/receipts、stateRoot、receiptsRoot、tx gossip 或共识执行语义。按“停止扩散 fixture/gateway/BAL/JSON-RPC，聚焦 Ethereum 主网长期同步 v1”的范围约束，SUPERVM 本轮不新增对应 JSON-RPC 面。真实产品入口三段验证使用 `NOVOVM_NODE_MODE=eth_rlpx_sync` 从完整 `current=25279644` 推进到 `25279800` header-only，再恢复 `25279800` body/receipt 并推进到 `25279806` header-only，最后恢复为 `current=25279806 body_available=true receipt_available=true`；剩余瓶颈仍是公网 forward `GetBlockHeaders` ready peer 接受度和 `too_many_peers`/timeout/close churn，不是新的 geth 协议语义差异，也不能据此声明 24h geth-like 长同步完成。

28. 2026-06-10 runtime native head 单调性收口：真实长同步从完整 `25279806` 前进到 `25279934` 时暴露本地状态错位：并发公网 peer 对同一 `GetBlockHeaders start=25279807` 返回不同长度批次后，runtime `current_block` 保持 `25279934`，但全局 native header/head snapshot 被较低批次回写成 `25279870`，随后 checkpoint/head store 写成 `current=25279934/header=25279870/body=25279870/receipt=25279870`。这会让重启后的 current 与可验证 material 不一致，属于长期同步正确性缺口。本轮修正为：runtime native header/head snapshot 不接受低高度回退，同高度才按较新 observed 时间刷新；checkpoint/head/history store 的 `current_block` 以实际写入 header material 为准，`highest_block` 继续保留远端高水位；restore 返回真实 header 高度，不再用错误 store current 覆盖。回归 `cargo test -p novovm-network runtime_status::tests::native_header_and_head_snapshots_do_not_regress_to_lower_height -- --exact --nocapture`、`cargo test -p novovm-node mainline_evm_cli_tests::eth_rlpx_sync_checkpoint_current_does_not_exceed_header_number_v1 -- --exact --nocapture`、`cargo test -p novovm-node mainline_evm_cli_tests::eth_rlpx_native_stores_current_tracks_material_header_v1 -- --exact --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo fmt --check`、`cargo check --workspace` 通过。真实产品入口验证先从旧错位 store 恢复到一致的 `current=25279934/header=25279934`，补齐 body/receipt 后继续推进到 `25279950` header-only；第二段再恢复 `25279950` body/receipt，最终 head store 与 checkpoint 均为 `current=25279950/header=25279950/body=25279950/receipt=25279950`。剩余问题仍是公网 ready peer 接受度和 forward headers 返回稳定性，不是本地状态错位。
29. 2026-06-10 RLPx 默认 fanout 范围收敛：采纳外部参考中“停止扩散 fixture/gateway/BAL/JSON-RPC、不要把公网 peer admission 调参做成工程黑洞”的边界，本轮只改长期同步入口的响应性。ready-session sync peers 现在也受 tick budget 截断，并在 runtime snapshot 输出 `skipped_sync_budget_peers`；默认 adaptive bootstrap fanout 不再自动升满 geth-style active window 50，而是保持默认 8，显式设置 `NOVOVM_ETH_RLPX_SYNC_TARGET_FANOUT` / `NOVOVM_ETH_RLPX_ADAPTIVE_BOOTSTRAP_FANOUT` 时仍可做高 fanout 探测。回归 `cargo test -p novovm-network real_rlpx_sync_peers_are_bounded_by_tick_budget_v1 -- --nocapture`、`cargo test -p novovm-network real_rlpx_parallel_bootstrap_bounds_slow_connects_v1 -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo fmt --check`、`cargo check --workspace` 通过。真实入口验证从完整 `25279982` 继续补齐到 `25279998`、`25280030`，随后默认 fanout-8 run 正常退出并推进到 header-only `25280158`，runtime snapshot 显示 `scheduledBootstrap=8/attemptedBootstrap=8`；显式 fanout-16 recovery 仍只发出 `missing_bodies_requested`，未收到 body，失败集中在 `too_many_peers`、timeout、MAC mismatch。当前结论：默认产品入口不再无限扩大 admission 压力，但 `25280158` body/receipt 仍未恢复，剩余瓶颈明确是公网 body-serving peer 获取和长期 ready peer admission；不能声明已经像 geth 一样 24h 无差别主网同步。
...
34. 2026-06-10 capacity reject cache 收口：继续按 `Ethereum 主网长期同步 v1` 收敛，修复重启续同步中的公网 overload 反复踩坑。旧 peer endpoint cache 只保存 endpoint 顺序；进程重启后内存里的 `too_many_peers` cooldown 丢失，刚被容量拒绝的 peer 会重新按 pristine bootstrap candidate 参与选择。本轮保持 cache schema `supervm-eth-rlpx-peer-endpoint-cache/v1` 兼容，新增可选 `capacity_rejects` 元数据：写 cache 时持久化未过期的 `too_many_peers` cooldown，读 cache 后先恢复到 runtime peer reputation，再执行 startup merge/write。旧无 `capacity_rejects` 字段的 v1 cache 仍可加载。回归 `cargo test -p novovm-node eth_rlpx_peer_endpoint_cache_ -- --nocapture`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo check --workspace` 通过。真实验证第一段 3 tick 写入 `capacity_rejects=19`；第二段重启后增至 `28`；第三段带 `NOVOVM_NODE_VERBOSE=1` 明确打印 `restored_capacity_rejects=28`，runtime `cooldown_count=36`；随后 6 tick 继续追高仍未拿到 ready peer，cache 扩到 `capacity_rejects=59`，当前仍为完整 `25280158/highest=25281414`。因此本阶段关闭的是“重启后重复选择刚 overload 的 public peer”缺口，长期同步剩余瓶颈仍是有效 forward-header peer 获取和公网 ready peer admission。

35. 2026-06-10 permanent reject cache 收口：继续只修 `Ethereum 主网长期同步 v1` 的重启续同步链路。本轮发现旧 cache 会丢失 pristine peer 的永久拒绝状态，导致 legacy `eth/66-68` capability-only peer 或畸形 Status peer 在重启后再次占用 bootstrap/admission 窗口。现在 cache schema 仍为 `supervm-eth-rlpx-peer-endpoint-cache/v1`，新增可选 `permanent_rejects` 元数据，短期持久化 `rlpx_eth_capability_not_found`、`rlpx_eth_status_fields_short`、`rlpx_eth_status_fork_id_not_list`、`rlpx_eth_status_fork_id_fields_short` 这类不可用 peer；读取 cache 后先恢复到 runtime peer lifecycle，再执行候选 merge/write。回归 `cargo test -p novovm-node eth_rlpx_peer_endpoint_cache_ -- --nocapture`、`cargo test -p novovm-network status_fork_id -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo fmt --check`、`cargo check --workspace`、`git diff --check` 通过。真实验证第一段 3 tick 将 `permanent_rejects` 从 0 写到 3；第二段带 `NOVOVM_NODE_VERBOSE=1` 明确打印 `restored_permanent_rejects=3`；第三段 2 tick 从 cache 恢复后把候选从 508 剪到 506，并将 `permanent_rejects` 增到 5。当前 head 仍完整停在 `current=25280158/highest=25281414`，所以本阶段只关闭“重启后重复踩不可用 public peer”的缺口，不声明 24h geth-like Ethereum 主网长期同步完成。

36. 2026-06-10 capacity reject backoff 收口：真实 6 tick 复验显示，过滤 legacy peer 后已能短暂拿到 ready peer 并发出 `GetBlockHeaders start=25280159 max=192`，但该 peer 在未返回 headers 前以 `too_many_peers` 断开；旧逻辑对 `too_many_peers` 固定约 60 秒冷却，且 cache 只保存 `reject_until`，不保存容量拒绝次数，进程重启后会把重复饱和 peer 退回“一次拒绝”窗口。本轮只修该长期同步链路：`too_many_peers` disconnect 按 `disconnect_too_many_peers_count` 做 1m/2m/4m/8m/15m 封顶退避；peer cache 的 `capacity_rejects` 增加可选 `disconnect_too_many_peers_count` 字段，旧 cache 缺字段按 1 归一化；启动恢复时按 cache 的拒绝截止时间和次数恢复 runtime peer lifecycle，而不是重新观察一次导致计数失真。回归 `cargo test -p novovm-network capacity_reject -- --nocapture`、`cargo test -p novovm-node eth_rlpx_peer_endpoint_cache_ -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo fmt --check`、`cargo check --workspace`、`git diff --check` 通过。真实 4 tick 验证本轮只遇到新容量拒绝 peer，cache 写出新字段且 `max_capacity_count=1`，head 仍完整停在 `current=25280158/highest=25281565`。因此本阶段只减少 repeated capacity reject churn，不声明长期主网同步完成。

37. 2026-06-10 material success cache / forward bootstrap prefix 收口：参考意见中“停止扩散 fixture/BAL/gateway/JSON-RPC、聚焦长期同步 v1”的边界有效，本轮只修重启续同步的 productive peer 复用。旧 cache 不保存 header/body-serving 成功 peer 的 material 历史，重启后 `157.90.35.166:30303` 这类刚服务过 body/receipt 的 public peer 会重新靠普通 rotation 竞争；同时即使 node 侧把显式 ENODE 置于候选前缀，network bootstrap 在 current head 已完整、进入 forward headers 场景时仍可能绕过候选前缀。本轮新增可选 `material_successes` cache 元数据，保存短期 header/body response count 与 head height，启动时恢复到 runtime peer reputation；显式 ENODE 继续 pin 到候选前缀；forward bootstrap 在没有 body/header/source 更强历史信号时尊重调用方候选前缀。回归 `cargo test -p novovm-network bootstrap_selection_preserves_candidate_prefix_for_forward_sync_without_history -- --nocapture`、`cargo test -p novovm-node eth_peer_endpoint_pin_prefix_keeps_explicit_peers_first_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_peer_endpoint_cache_ -- --nocapture`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo fmt --check`、`cargo check --workspace`、`git diff --check` 通过。真实入口第一段带显式 ENODE 启动，tick 1 对 `peer=5398490727121101794 endpoint=157.90.35.166:30303` 发出 `GetBlockHeaders start=25280159`，tick 2 收到 `headers=128/bodies=1/receipts=1`，head 从完整 `current=25280158/highest=25281565` 推进到完整 `current=25280286/highest=25281730`；cache 写出 `material_successes`，该 peer `header_response_count=128/body_response_count=1`。第二段不带显式 ENODE 重启，打印 `restored_material_successes=1`，继续选择同一 peer，最终推进到完整 `current=25280414/highest=25281736`，并把 material count 更新到 `header_response_count=144/body_response_count=2`。因此本阶段关闭的是“productive public peer 重启后丢失/显式前缀不生效”的续同步缺口；仍未完成 24h geth-like Ethereum 主网长期同步。

38. 2026-06-10 productive close retry 收口：连续真实短跑证明 material success cache 后能够继续推进，但成功服务 headers/body/receipt 的 public peer 在普通 close/EOF 后仍会进入通用 2.5s 退避，短 tick 下会空转一到两轮；同时上一阶段 forward bootstrap 的 candidate-prefix fallback 曾经过强，可能让刚有 capacity-reject 记录的前缀 peer 压过 fresh candidate。本轮只修生命周期选择：已经贡献过 header/body/sync material 的 peer 遇到普通 close/EOF 时使用 750ms 短重试；`too_many_peers` / capacity reject 仍保持 1m 起步长退避；fallback candidate-prefix 只在同为健康候选时优先，近期失败/容量拒绝不会被前缀强行选中。回归 `cargo test -p novovm-network productive_ -- --nocapture`、`cargo test -p novovm-network capacity_reject -- --nocapture`、`cargo test -p novovm-network bootstrap_selection_preserves_candidate_prefix_for_forward_sync_without_history -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo fmt --check`、`cargo check --workspace`、`git diff --check` 通过。真实入口复验从完整 `current=25280798/highest=25281777` 启动，tick 1/2 通过 `157.90.35.166:30303` 推进到完整 `25280814`，tick 3/4 再推进到完整 `25280830/highest=25281810`；期间 material peer 在 tick 1/3/4 被复用，普通 close 后 runtime 显示短 cooldown，最终仍因 public `too_many_peers`/admission churn 停在完整 `25280830`。因此本阶段只关闭 productive peer 普通断线后的过长空转，不声明 24h geth-like 主网长期同步完成。

39. 2026-06-10 material-success endpoint prefix 收口：继续按外部参考意见收敛，不扩散 fixture/BAL/gateway/JSON-RPC，本轮只修 `Ethereum 主网长期同步 v1` 的重启续同步链路。上一轮 cache 已能保存 `material_successes`，但真实复验发现成功 peer `157.90.35.166:30303` 仍可能被写在普通 `endpoints` 列表数百位之后；重启时虽恢复了 runtime material reputation，但启动候选仍先消耗大量 fresh/public candidate。本轮在写 peer endpoint cache 时，将未处于 `capacity_rejects` 或 `permanent_rejects` 的 material-success endpoint 前置到持久化 endpoint prefix，保持 schema `supervm-eth-rlpx-peer-endpoint-cache/v1` 兼容。回归 `cargo test -p novovm-node eth_rlpx_peer_endpoint_cache_ -- --nocapture`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo test -p novovm-network productive_ -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo fmt --check`、`cargo check --workspace`、`git diff --check` 通过。真实入口第一段 2 tick 无显式 ENODE 启动，恢复 `restored_material_successes=1`，tick 2 向 `peer=5398490727121101794 endpoint=157.90.35.166:30303` 发出 headers 请求并把 cache 重写为该 peer 位于 `endpoints[0]`；第二段 6 tick 重启后 tick 1 立即向同 peer 发 `GetBlockHeaders start=25280847 max=16`，tick 2 收到 `bodies_received` 与 `receipts_received`，完整 head 从 `current=25280846/highest=25281859` 推进到 `current=25280862/highest=25281865`，且 `body_available=true/receipt_available=true`。后续 tick 仍因公网 `too_many_peers`、timeout、legacy capability 和 ingest close 回到 admission churn，所以本阶段只关闭“material-success 已保存但 cache endpoint 顺序未前置”的本地缺口，不声明 24h geth-like Ethereum 主网长期同步完成。

40. 2026-06-10 state-phase header window 收口：真实 20 tick 复验显示，material-success endpoint 前置后已能继续推进，但当完整 head 从 `25280862` 推进到 `25280990` 后，native phase 进入 `State`，下一次 forward `GetBlockHeaders start=25280991` 被本地 planner 固定为 `max=16`。当前仍落后远端约 900 块，过早把 scarce ready peer 窗口压成 16 会拖慢长期追高。本轮只改 state-lag header chase：当 `State` phase 但 `header_lag > 128` 且当前 head body 已可用时，header 请求使用完整 state-phase pull window；缺 body 或近头 state/snap 路径仍保持小窗口，避免抢掉当前 head recovery 和 snap 近头工作。回归 `cargo test -p novovm-network native_state_ -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo fmt --check`、`cargo check --workspace`、`git diff --check` 通过。真实入口 6 tick 从完整 `current=25280990/highest=25281895` 恢复，tick 1 发出 `GetBlockHeaders start=25280991 max=64`，tick 2 收到 `headers=64/bodies=1/receipts=1` 并推进到完整 `25281054`；tick 3 再发 `GetBlockHeaders start=25281055 max=64`，tick 4 再收到 `headers=64/bodies=1/receipts=1` 并推进到完整 `25281118/highest=25281919`。后续仍回到公网 `too_many_peers`、timeout 和 legacy capability churn，所以本阶段只关闭 state-lag band 内 headers window 过小的问题，不声明 24h geth-like Ethereum 主网长期同步完成。

41. 2026-06-10 public sync request interval 收口：继续只修 `Ethereum 主网长期同步 v1` 产品入口。上一轮 state-phase window 放大后，24 tick 复验从完整 `25281118` 只推进到 `25281182`，之后 tick 3 出现 `ready=1 connected=0 sync_requests=0`：同一 material peer 仍在 runtime ready/live session 可用窗口内，但公网产品入口沿用核心默认 `sync_request_interval_ms=1000`，而 tick sleep 默认 600ms，导致下一段 headers 请求晚一 tick 才可能发出，peer 经常在等待中关闭。本轮保持核心库默认不变，只把直接 Rust 产品入口 `NOVOVM_NODE_MODE=eth_rlpx_sync` 的默认 `NOVOVM_ETH_RLPX_SYNC_REQUEST_INTERVAL_MS` 设为 250ms，仍允许 env 覆盖。回归 `cargo test -p novovm-node eth_rlpx_public_sync_runtime_defaults -- --nocapture`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo fmt --check`、`cargo check --workspace`、`git diff --check` 通过。真实入口 8 tick 从完整 `current=25281182/highest=25281940` 恢复，tick 1 发出 `GetBlockHeaders start=25281183 max=64`，tick 2 收到 `headers=64/bodies=1/receipts=1` 推进到完整 `25281246`；tick 3 没有再空等，立即发出 `GetBlockHeaders start=25281247 max=64`，tick 4 再收到 `headers=64/bodies=1/receipts=1` 推进到完整 `25281310/highest=25281980`。后续仍受公网 `too_many_peers`、timeout 和 legacy capability churn 影响，所以本阶段只关闭 productive peer 请求间隔过长的问题，不声明 24h geth-like Ethereum 主网长期同步完成。

42. 2026-06-10 stale-ready 与 effective state-lag adaptive 收口：继续只修 `Ethereum 主网长期同步 v1`。真实 follow-up 从完整 `25281438` 推进到 `25281502` 后再次暴露本地空转：tick 3 输出 `ready=1 connected=0 sync_requests=0 native_phase=discovery`，说明 runtime lifecycle 仍把 peer 视为 ready，但真实 `ETH_FULLNODE_NATIVE_RLPX_SESSIONS` 已无 live TCP/RLPx session，plan 因此把不存在的 session 计入 sync/active session。另一个同轮问题是 transport failure 后 adaptive header batch 按配置上限 `192` 回退到 `96`，但 state-lag forward header 的实际窗口是 64，所以第一次回退对下一次真实请求无效。本轮在 real worker plan 前对 runtime-ready peer 与 live session map 做 reconciliation，无 live session 即标记 session closed；同时产品入口 adaptive 在高 lag、当前 head material 完整且 phase 为 `State` 或掉线后的 `Discovery` 时，以实际 64-header state-lag 窗口作为失败回退起点。回归 `cargo test -p novovm-network real_rlpx_worker_reconciles_stale_runtime_ready_without_live_session_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_state_lag_transport_failure_backs_off_effective_header_window_v1 -- --nocapture` 通过。真实入口 10 tick 从完整 `current=25281566/highest=25282074` 继续，tick 1/2 两个 public peer 合计 materialize `headers=128/bodies=2/receipts=2`，当前完整推进到 `25281630`；后续 `rlpx_frame_header_read_failed` 后输出 `eth_rlpx_adaptive_batch ... headers_old=192 headers_new=32`，下一 tick 再 `32 -> 16`，并且 stale-ready 形态变为 `ready=0 connected=0 sync_requests=0` 后正常 refresh。剩余仍是公网 `too_many_peers`、timeout、pre-auth close 和 body/header-serving peer 获取，不声明 24h geth-like Ethereum 主网长期同步完成。

43. 2026-06-10 complete-head batch restore 收口：继续只修 `Ethereum 主网长期同步 v1` 的公网产品入口。上一轮真实 14 tick 从完整 `25281630` 推进到 `25281726` 后暴露恢复条件过宽：tick 13 有 `headers=32/bodies=1/receipts=0` 这类 partial material，`current` 未前进且新窗口 receipt 未完成，但 adaptive 立即 `headers_old=32 headers_new=64`、`bodies_old=64 bodies_new=128`，下一 tick 又遇到 `rlpx_frame_header_read_failed`。本轮保留 partial material 对 stall/refresh 诊断的价值，但 batch restore 只允许在 `current_block` 相比上一完整进展块真正前进，且当前 head 的 header/body/receipt 都可用时触发；partial header/body 或 receipt-only 恢复不再放大刚回退的公网窗口。回归 `cargo test -p novovm-node eth_rlpx_batch_restore_requires_complete_head_advance_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture` 通过。真实入口 12 tick 从完整 `current=25281726/highest=25282149` 继续，tick 1/2 推进到完整 `25281790`，tick 3 在 public session close 后只回退 `headers_old=192 headers_new=32/bodies_old=128 bodies_new=64`，后续没有完整 head 前进时不再恢复放大。剩余仍是公网 admission churn，不声明 24h geth-like Ethereum 主网长期同步完成。

44. 2026-06-10 长期同步 v1 范围收敛与 refresh clock 收口：采纳当前阶段判断，停止继续扩大 EVM fixture、gateway、BAL、JSON-RPC 和产品叙事范围；本阶段唯一主线是 `NOVOVM_NODE_MODE=eth_rlpx_sync` 像 geth 一样长期加入 Ethereum 主网的同步链路。上一轮真实 14 tick 暴露出另一个本地误判：complete head 在 tick 10 推进到 `25281886` 后，后续仅有 partial material/ready churn，但 forward stalled refresh 仍按普通 `last_sync_progress_tick` 计时，导致 partial body/receipt 活动会延后下一轮候选刷新。本轮将 forward refresh 的 stall clock 改为最后一次完整 head 进展 tick；只要 `highest > current` 且完整 head 没有按间隔前进，就刷新/扩候选，不再被 partial material 拖延。同时直接公网入口把 runtime snapshot 的 pending tx 明细默认上限从通用 `2048` 收敛为 `NOVOVM_ETH_RLPX_RUNTIME_PENDING_TXS=128`，summary 统计保留，降低长期 tick 的本地 JSON 写入压力。回归 `cargo test -p novovm-node eth_rlpx_forward_refresh_stall_uses_complete_head_progress_tick_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_public_sync_runtime_defaults_honor_entry_fanout_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_public_sync_runtime_defaults_preserve_smaller_snapshot_cap_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture` 通过。真实入口 6 tick 从完整 `current=25281950/highest=25282228` 启动，tick 2 收到 `headers=128/bodies=2/receipts=2` 并推进到完整 `25282014`，tick 4 继续到 `25282078` 但 receipt 尚缺，tick 5 触发 `body_recovery_stalled_expand`，本轮 snapshot 记录 `native_pending_txs=128`，文件约 `9.08MB`。剩余仍是公网 `too_many_peers`、timeout、pre-auth close、material-serving peer 获取和 24h 重启续跑验证，不声明 geth-like 长期主网同步完成。

45. 2026-06-10 near-head finalize budget 收口：继续只修 `Ethereum 主网长期同步 v1`。18 tick 真实复验先补齐 `25282078` receipts，再推进到 `25282158`，但发现进入 near-head `Finalize` 后仍落后约 121 块时下一次 `GetBlockHeaders` 固定为 `max=16`；后续把公网窗口拆得过碎，容易在多轮等待中丢失 ready peer。根因是 runtime sync pull window 仍使用固定 phase 常量，没有使用已经下发到 real worker 的 `EthFullnodeBudgetHooksV1.sync_pull_finalize_batch`。本轮把 runtime pull window 改为读取 per-chain active budget hooks；直接产品入口默认 `NOVOVM_ETH_RLPX_FINALIZE_HEADERS_BATCH=64`，并在 finalize/discovery near-head transport failure 时按实际 64-header 窗口回退到 32，而不是无效的 `192 -> 96`；runtime snapshot 也写入当前 plan 的实际 budget，避免诊断误导。回归 `cargo test -p novovm-network plan_sync_pull_window_ -- --nocapture`、`cargo test -p novovm-network rlpx_headers_request_batch_respects_runtime_budget_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_finalize_transport_failure_backs_off_effective_header_window_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo check -p novovm-network -p novovm-node` 通过。真实入口先从 `25282158` 补齐 receipts 并发出 `GetBlockHeaders max=64`，推进到完整 `25282222`；后续 10 tick 从完整 `25282222/highest=25282320` 启动，tick 2 收到 `headers=128/bodies=2/receipts=2` 并推进到完整 `25282286`，gap 34 时请求 `GetBlockHeaders max=34`，public close 后 adaptive 输出 `headers_old=192 headers_new=32`，snapshot 显示 `runtime_headers_batch=32/runtime_finalize_batch=32`。剩余仍是公网 `too_many_peers`、timeout、pre-auth close、wrong-network/legacy peers 和 24h 重启续跑验证，不声明 geth-like 长期主网同步完成。

46. 2026-06-10 product current-head body-first 收口：继续只修 RLPx 长期同步链路。24 tick 真实复验从 `25282286` 推进到 `25282360`，但暴露 near-head 本地策略缺口：当 `GetBlockHeaders` 一次返回到已知 `highest` 时，旧逻辑认为“不是追高”，于是立即对多个 headers 发送批量 `GetBlockBodies`；公网 peer 多轮 partial body retry 后断开，当前 head 反而需要后续 body recovery 才补齐。本轮把 header follow-up body 选择收窄为产品公网预算路径优先：直接公网入口的 active `sync_pull_finalize_batch` 高于核心默认时，任何多 header 响应只优先请求最新/current head body；仍在追高时也保持 current-head-first。核心默认小 batch 语义不被改宽。全部 header 仍写入 canonical/history，历史 body 缺口留给 bounded missing-body recovery。另一个同轮修复是：当前 head 已完整且 `current==highest` 时，历史 backfill 的 transport failure 不再回退 forward header window，避免纯 backfill 失败惩罚下一轮追高。回归 `cargo test -p novovm-network rlpx_large_header_batch_import_requests_current_body_only_at_highest_v1 -- --nocapture`、`cargo test -p novovm-network rlpx_small_header_batch_import_requests_current_body_only_at_highest_v1 -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_idle_backfill_failure_does_not_backoff_header_window_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo check -p novovm-network -p novovm-node` 通过。真实入口 8 tick 从完整 `25282431/highest=25282431` 附近启动，tick 1 发出 `GetBlockHeaders start=25282432 max=35`，tick 2 只出现 `bodies_requested blocks=1` 的 current-head follow-up 并推进到完整 `25282466`；后续历史 backfill failure 只回退 bodies `128 -> 64`，forward headers 保持 `192 -> 192`。剩余仍是公网 `too_many_peers`、timeout、pre-auth close、material-serving peer 获取和 24h 重启续跑验证，不声明 geth-like 长期主网同步完成。

47. 2026-06-10 idle-head probe 与 adaptive admission 默认值收口：继续只修 `Ethereum 主网长期同步 v1`。真实 24 tick 复验先从完整 `25282466` 推进到完整 `25282495`，随后 `current==highest` 进入 idle；但真实链头还在前进，本地因为没有 fresh status/head probe，后续 22 tick 都停在 stale highest。本轮增加 idle-head probe 判定：当 `current==highest`、当前 head/body/receipt 完整、且连续 stalled interval 没有 ready peer 或 status update 时，按 `head_probe_stalled_*` 刷新/扩候选；同时直接产品入口默认 adaptive bootstrap fanout 从“等于启动 fanout 8”改为“启动仍 8，stalled admission 后可提升到 bounded 32”，上限仍受 `NOVOVM_ETH_RLPX_MAX_PEERS` 控制。回归 `cargo test -p novovm-node eth_rlpx_idle_head_probe -- --nocapture`、`cargo test -p novovm-node eth_rlpx_peer_refresh_plan_ -- --nocapture`、`cargo test -p novovm-node eth_rlpx_adaptive_bootstrap_fanout_raises_only_when_admission_stalled_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo check -p novovm-network -p novovm-node`、`cargo fmt --check`、`git diff --check` 通过。真实入口 12 tick 从完整 `25282522` 启动，tick 2 推进到完整 `25282549/highest=25282549`；tick 6 在无 ready/status 的完整 idle head 上输出 `eth_rlpx_adaptive_fanout old=8 new=32` 和 `eth_rlpx_peer_refresh reason=head_probe_stalled_expand`，tick 7 重新获得 status 并把 `highest` 刷到 `25282553`，tick 9/10 继续发 headers/body/receipts 并推进到完整 `25282551`。剩余仍是公网 `too_many_peers`、timeout、pre-auth close、material-serving peer 获取和 24h 重启续跑验证，不声明 geth-like 长期主网同步完成。

48. 2026-06-10 product header-floor 收口：继续只修 `Ethereum 主网长期同步 v1`。30 tick 真实复验显示 idle-head probe 和 adaptive fanout 已生效，但公网 request failures 会把 forward header batch 连续压到 `1/2`，后半段只能发 `GetBlockHeaders max=1/2`，而远端 `highest` 继续增长，追高效率被本地 adaptive floor 拖慢。本轮不改 wire 语义、不改 body/receipt 精准恢复，只把直接公网产品入口默认 `NOVOVM_ETH_RLPX_ADAPTIVE_HEADERS_MIN_BATCH` 从 `1` 提到 `4`；`NOVOVM_ETH_RLPX_ADAPTIVE_BODIES_MIN_BATCH` 仍默认 `1`，需要窄探测时 operator 仍可显式把 header floor 降回 `1`。回归 `cargo test -p novovm-node eth_rlpx_product_header_floor_keeps_forward_probe_above_single_block_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_adaptive_public_sync_batch_ -- --nocapture`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo check -p novovm-network -p novovm-node`、`cargo fmt --check`、`git diff --check` 通过。真实入口 18 tick 从完整 `25282583` 启动，tick 2 推进到完整 `25282609`，tick 6 继续触发 `head_probe_stalled_expand`，后续 near-head request 使用 `GetBlockHeaders max=4` 而不是 `max=1`，tick 14 推进到 `25282615`，tick 18 补齐 receipts 并输出 `headers_old=16 headers_new=32/bodies_old=4 bodies_new=8 reason=complete_head_progress_restore`。剩余仍是公网 `too_many_peers`、timeout、pre-auth close、material-serving peer 获取和 24h 重启续跑验证，不声明 geth-like 长期主网同步完成。

49. 2026-06-10 long-sync follow-up 诊断收口：本轮不改 Rust 代码，只验证上一阶段修复是否持续成立。36 tick 真实入口从完整 `25282615` 推进到完整 `25282663`，随后生成 header-only `25282664`；16 tick 续跑在 tick 1/2 立即发出 `missing_bodies_requested` 并补齐 `25282664` body/receipt，再推进到完整 `25282681`；24 tick 续跑继续从完整 `25282681` 推进到完整 `25282698`，期间 `25282698` receipt-missing 由 `missing_receipts_requested` 在 tick 15/16 恢复。header probe 默认 floor 生效，未再出现产品 forward `GetBlockHeaders max=1`；idle-head probe 仍能在 `current==highest` 且无 ready/status 时触发 `head_probe_stalled_expand`，adaptive fanout 升到 32。最终停在 `current=25282698/highest=25282704`、head/body/receipt 完整，但无 ready peer；runtime snapshot 显示 `sync_target_fanout=32`、`active_native_peer_hard_limit=1024`、约 599 candidates、`ready_count=0`，失败集中在公网 `too_many_peers`、timeout、pre-auth close。结论：短窗口重启续跑、header-only recovery、receipt recovery 和 stale-head probe 都可工作；剩余未封口的是公网长期 admission/cooldown 与 24h soak，不声明 geth-like 长期主网同步完成。

50. 2026-06-10 reputation-ordered startup/refresh 收口：继续只修 `Ethereum 主网长期同步 v1` 的公网 admission/cooldown 链路。上一轮 snapshot 显示 cache/候选池里有大量 `too_many_peers` 冷却节点；本轮不新增外部脚本、不改 fixture/BAL/gateway，只把 peer endpoint cache 排序改成：有 active capacity/permanent reject 的 endpoint 持久化时下沉到 productive/fresh endpoint 后面，并在启动、refresh 生成 candidate list 后立即按恢复的 runtime reputation 重排，再构造 worker。新增 `eth_rlpx_peer_endpoint_cache_demotes_capacity_reject_prefix_v1`，回归 `cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo check -p novovm-node`、`cargo fmt --check` 通过。真实入口 16 tick 从完整 `25282698` 重启，tick 1/2 直接获得 2 个 ready peer，推进到完整 `25282752`；tick 13/14 再次获得 ready/status 并推进到 `25282760`，结束时 receipt 未补齐；12 tick 续跑 tick 1/2 补齐 `25282760` receipts，tick 4 推进到 header-only `25282766`，tick 7/8 补齐 `25282766` body/receipts。最终停在 `current=25282766/highest=25282771` 且当前 head/body/receipt 完整，仍未完成长期追高；剩余仍是公网 admission/cooldown churn 与 24h soak，不声明 geth-like 长期主网同步完成。

51. 2026-06-10 productive capacity retry / cache pool restore 收口：继续收敛公网 admission/cooldown，不扩 fixture/BAL/gateway。普通 `too_many_peers` 仍保留长退避；但对已有 header/body/sync contribution 的 productive peer，capacity reject 首次改为较短重试窗口并随重复次数递增，避免少数真正能供料的 peer 因一次 sync 阶段容量拒绝被踢出 60 秒以上。另一个实测缺口是 runtime refresh 可把候选池扩到 540/573，但重启时 `eth_rlpx_cache_warmed_candidate_limit_v1` 会把 cache pool 压回 512；本轮把 cache-warmed 上限改为 1024，使重启续跑保留已经扩出来的公网 endpoint 池。新增/更新 `productive_capacity_reject_uses_short_retry_backoff`、`repeated_productive_capacity_rejects_backoff_but_stay_below_plain_capacity_floor`、`eth_rlpx_cache_warmed_candidate_limit_preserves_expanded_pool_v1`。回归 `cargo test -p novovm-network capacity_reject -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo check -p novovm-network -p novovm-node`、`cargo fmt --check` 通过。真实入口 24 tick 从完整 `25282766` 推进到完整 `25282824`，期间 `25282811`、`25282821`、`25282824` body/receipts 均恢复；12 tick 续跑从完整 `25282824` 推进到完整 `25282831`；cache pool restore 后 8 tick 重启以 `candidates=546` 起步而非 512，tick 1 得到 4 ready peers，tick 2 推进到完整 `25282848`，tick 6 refresh 到 569 candidates，最终停在 `current=25282848/highest=25282852` 且当前 head/body/receipt 完整。结论：重启候选池保留和 productive peer 复用改善成立，但仍未完成 24h geth-like 主网长期同步。

52. 2026-06-10 idle historical missing-body backfill 收口：继续只修 `Ethereum 主网长期同步 v1`，不扩 fixture/BAL/gateway/JSON-RPC。上一轮真实续跑从完整 `25282848` 追到 `25282881` 时，历史缺 body 回填仍会在 idle/complete-head 场景一次请求 16 个 body；公网 peer 对这种历史批量经常 partial response 后 timeout，既冷却 productive peer，也拖慢后续 near-head 追高。本轮把 idle historical missing-body backfill 上限从 16 收敛为 4，同时保留 current/latest missing head 的优先恢复名额；追高中的 current-head recovery 仍保持 1。新增 `rlpx_missing_body_idle_backfill_caps_historical_batch_v1`，回归 `cargo test -p novovm-network missing_body -- --nocapture --test-threads=1`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo check -p novovm-network -p novovm-node`、`cargo fmt --check` 通过。真实入口 12 tick 先补齐 `25282881` receipts，随后从 `25282881/highest=25282916` 推进到完整 `25282920`；本轮实际 body 请求均为 `blocks=1` 的 current-head 请求，未再出现 16-block 历史 backfill burst。该修复只降低 idle 历史回填对公网 ready peer 的占用，不声明 24h geth-like 主网长期同步完成。

53. 2026-06-10 recovery in-flight 去重与 empty body response 收口：继续只修 `Ethereum 主网长期同步 v1`。24 tick 真实续跑先从完整 `25282920` 推进到完整 `25282954`，但暴露两个本地浪费：缺 body/receipt recovery 会在不同 ready session 上重复请求同一 hash；并且某个公网 peer 对单块 `GetBlockBodies` 连续返回 `blocks=0` 时，旧逻辑会在同一 peer 上持续 partial retry 十几次，最后触发 `subprotocol_error`。本轮增加短 TTL 的 body/receipt in-flight registry，普通 follow-up body/receipt 请求、missing-body recovery、missing-receipt recovery、partial retry 都登记 hash；response、disconnect 和 request 清理会释放登记，missing recovery 只跳过其他 peer 已在处理的 hash，不阻止同 peer 正常完成请求。同时对 pending hash 的空 `BlockBodies` response 直接释放该 peer，不再同连接紧循环。新增 `rlpx_recovery_inflight_filter_skips_other_peer_hashes_v1`、`rlpx_block_bodies_empty_response_does_not_retry_same_peer_v1`。回归 `cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo check -p novovm-network -p novovm-node`、`cargo fmt --check`、`git diff --check` 通过。真实入口 12 tick 从完整 `25283028` 推进到 `25283053/highest=25283058`，当前 head body/receipt 完整；本轮 missing body/receipt recovery 为单 peer 请求，未复现先前同 peer `blocks=0` partial-body 重试风暴。该修复降低公网 ready peer 浪费和本地 tight-loop retry，不声明 24h geth-like 主网长期同步完成。

54. 2026-06-10 forward header exact-range in-flight 去重：继续只修 `Ethereum 主网长期同步 v1`。上一轮真实 24 tick 显示 recovery 去重后仍有本地浪费：多个 ready peer 会在同一 tick 对相同 near-head `start/skip/reverse` 发送 `GetBlockHeaders`，例如不同 peer 同时拉 `start=25283084` 或相邻 near-head 范围，后续 body/receipt 也被拆到重复 header 工作之后。本轮增加短 TTL 的 forward header in-flight key，以 `chain_id/start/skip/reverse` 去重；同一 peer 自己的 pending 不受阻止，matched header response、disconnect 或 failure 清理该 key。新增 `rlpx_header_inflight_filter_skips_duplicate_forward_start_v1`，回归 `cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo check -p novovm-network -p novovm-node`、`cargo fmt --check`、`git diff --check` 通过。真实入口 12 tick 从完整 `25283087` 启动，tick 1 有 2 个 ready peer 但只发出一个 `headers_requested start=25283088 max=26`，随后推进到完整 `25283113/highest=25283114` 并补齐 receipts；剩余仍是公网 `too_many_peers`/timeout churn 和一次严格 pooled-tx response rejection，不声明 24h geth-like 主网长期同步完成。

55. 2026-06-10 pooled-tx pending request guard 收口：继续只修 `Ethereum 主网长期同步 v1`，不扩 fixture/BAL/gateway/JSON-RPC。上一轮真实 run 暴露一次严格 pooled-tx response rejection；本地复核 `D:\WEB3_AI\go-ethereum` 的 `eth/protocols/eth/handlers.go` 和 tx fetcher 语义后，结论不是要放松 unsolicited response matching，而是 SUPERVM live session 会在已有 `GetPooledTransactions` pending 时被新的 `NewPooledTransactionHashes` 覆盖 `last_pooled_transactions_request_id` / pending hashes，导致先前合法响应变成 unexpected。本轮增加 `eth_fullnode_native_should_request_pooled_transactions_v1`：空 hash 不请求，且同一 live session 已有 pending pooled-tx request 时不再发第二个请求覆盖状态；收到匹配响应后仍由 pending request-id 校验清理 pending。新增 `rlpx_pooled_transactions_request_does_not_overwrite_pending_request_v1`，回归 `cargo test -p novovm-network rlpx_pooled_transactions_request_does_not_overwrite_pending_request_v1 -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo check -p novovm-network -p novovm-node`、`cargo fmt --check`、`git diff --check` 通过。真实入口 12 tick 从完整 `25283142` 推进到 `25283194/highest=25283198`，中途补齐 `25283190` receipts，未复现 `rlpx_pooled_transactions_unexpected_response` 或 `subprotocol_error`；结束时当前 head body 已可用但 receipt 尚缺，说明剩余仍是公网 body/receipt-serving peer 获取和 24h 重启续跑验证，不声明 geth-like 长期主网同步完成。

56. 2026-06-10 pooled-tx geth tracker window 收口：继续只修 `Ethereum 主网长期同步 v1`。24 tick 续跑先恢复 `25283194` receipts，又推进到 `25283225`，但 tick 19 复现 `rlpx_pooled_transactions_unrequested_hash`；同一 peer 后续仍能服务 headers/bodies/receipts，说明把 tx gossip response 中夹带的未请求 hash 作为 decode failure 会误伤主网同步 peer。对照本地 geth `p2p/tracker/tracker.go`，`PooledTransactions` response 只通过 request id、message code 和 `Size <= request.Size` 做 tracker fulfil；`eth/handler_eth.go` 随后把 txs 交给 tx fetcher，`TestTransactionFetcherOutOfBoundDeliveries` 覆盖 out-of-bound tx delivery。SUPERVM 因此把 pooled-tx response 校验改为 geth tracker window：仍要求本地 pending request_id，response item count 不得超过 requested count，重复 tx hash 仍拒绝；但匹配 request_id 后不再要求 response hash 必须是 requested hashes 的有序子集，同 size extra tx 会 materialize 为 remote pending tx。新增 `rlpx_pooled_transactions_response_uses_geth_tracker_window_v1`、`rlpx_pooled_transactions_ingests_geth_accepted_unrequested_hash_v1`，回归 `cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1` 通过 `72 passed`。这只移除 tx gossip 侧误断连接，不声明 24h geth-like Ethereum 主网长期同步完成。

57. 2026-06-10 product current-head body-followup 标志收口：继续只修 `Ethereum 主网长期同步 v1`。上一轮真实 run 在 tick 24 暴露 `headers_requested max=3` 后跟随 `bodies_requested blocks=3`，原因是 current-head-first 逻辑靠 `sync_pull_finalize_batch > default` 推断产品路径；adaptive backoff 把 finalize window 降回默认 16 后，产品入口标识失效。本轮在 `EthFullnodeBudgetHooksV1` 增加显式 `prefer_current_head_body_on_header_batch`，默认 core worker 保持 false，`novovm-node eth_rlpx_sync` 直接产品入口设置 true；选择逻辑改为：所有链在 `highest > latest` 追高中仍 current-head-first，只有产品标志或高于默认 finalize window 时，`highest == latest` 的 known-highest response 也只请求 latest/current head body。新增 `rlpx_product_header_batch_requests_current_body_at_known_highest_v1`，并在 `eth_rlpx_public_sync_runtime_defaults_honor_entry_fanout_v1` 断言产品入口启用该标志；回归 `cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1` 通过 `73 passed`，`cargo test -p novovm-node eth_rlpx_ -- --nocapture` 通过 `53 passed`。真实入口 12 tick 从 `25283275` 推进到 `25283309`；31-header 和 3-header near-head responses 均只发出 `bodies_requested blocks=1`，未复现 pooled-tx `unrequested_hash`；结束时当前 head receipt 仍待恢复，不声明 24h geth-like Ethereum 主网长期同步完成。
