...
- novovm-node direct RLPx sync entry: pass for finite live mainnet run via `NOVOVM_NODE_MODE=eth_rlpx_sync`, covers real Status -> Headers -> Bodies -> Receipts and native current advancing from 0 to 2048；默认候选池已从 Ethereum mainnet geth bootnodes 扩展到 geth DNS discovery `all.mainnet.ethdisco.net` ENR 解析，实测候选从 4 扩到 32；still does not claim DNS tree signature verification, full discv4 peer churn, or long-haul catch-up
