...
- Ethereum discv4 discovery gate: pass, covers signed discv4 Ping/Pong/FindNode/Neighbors packet build/parse, bootnode endpoint proof bonding, inbound Ping -> Pong reply, public IPv4 Neighbors -> `enode://` candidate materialization, and mixed IPv4/IPv6 Neighbors parsing with unsupported IPv6 skipped rather than failing the whole packet；discovery-only live run with 4 geth mainnet bootnodes returned `endpoints=9` with `neighbor_parse_errors=0`；discv4+DNS 16 tick live run returned `discv4 endpoints=29` plus `DNS endpoints=15`, then RLPx sync advanced to `current=1024/highest=25267957`；still does not claim full discv4 Kademlia table/random walk, discv5, or long-haul peer churn
...
- novovm-node RLPx adaptive candidate refresh gate: pass, 当无 ready peer、无可调度 bootstrap/sync 且所有候选都在 cooldown 时，产品入口会把候选上限扩到 `NOVOVM_ETH_RLPX_ADAPTIVE_CANDIDATE_PEERS_MAX` 并重建 worker；104 tick live run 从 32 自动扩到 64（tick 10）再扩到 128（tick 57），之后仍重新连上 peer 并推进到 `current=5120/highest=25267820`；still does not claim full discv4 peer churn or long-haul mainnet acceptance
