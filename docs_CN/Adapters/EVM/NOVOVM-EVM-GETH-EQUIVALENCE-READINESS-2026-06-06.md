...
- novovm-node direct RLPx sync entry: pass for finite live mainnet run via `NOVOVM_NODE_MODE=eth_rlpx_sync`, covers real Status -> Headers -> Bodies -> Receipts and native current advancing from 0 to 2048 against Ethereum mainnet static enode；still does not claim discovery/peer replacement or long-haul catch-up
