...
22. 2026-06-09 head-first body recovery 收口：receipt 恢复后，真实入口从 `25279039` 发出 `headers_requested start=25279040 max=16`，随后导入 `headers=16` 到 `current=25279055`，但 peer 在 body 阶段断开并留下 header-only head。旧 missing-body recovery 虽能恢复后缀，但按升序请求旧块在前；公网 peer 若只返回短前缀，可能先物化旧 body 而不是当前 head。现在追高场景的 missing-body recovery 保留当前 head 优先顺序，并把连续 header-only 后缀窗口扩到 16，使短 `BlockBodies` 响应也优先恢复当前 head。回归 `cargo test -p novovm-network rlpx_missing_body_recovery_ -- --nocapture --test-threads=1` 通过；真实 12 tick 验证从持久化 header-only `25279055` 启动，多次发出 `missing_bodies_requested ... blocks=16`，但公网 peer 仍未返回 bodies，失败集中在 `too_many_peers`、pre-auth close 和 ingest close。该阶段关闭 request order/window 缺口，但不声明 body-serving peer 获取或 24h geth-like 长期同步完成。
