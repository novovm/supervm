...
- novovm-node RLPx adaptive candidate refresh gate: pass, 当无 ready peer、无可调度 bootstrap/sync 且所有候选都在 cooldown 时，产品入口会先把候选上限扩到 `NOVOVM_ETH_RLPX_ADAPTIVE_CANDIDATE_PEERS_MAX` 并重建 worker；到达 adaptive 上限后，若仍全冷却，则按 `NOVOVM_ETH_RLPX_EXHAUSTED_REFRESH_INTERVAL_TICKS` 重新跑 discovery/DNS，同容量候选里只要出现新 endpoint 也会重建 worker；104 tick live run 从 32 自动扩到 64（tick 10）再扩到 128（tick 57），之后仍重新连上 peer 并推进到 `current=5120/highest=25267820`；新增同容量 refresh 纯函数 gate 覆盖“长度不变但 endpoint 变化”场景；44 tick live run 未触发全冷却 refresh，因为节点重新拿到 ready peer 并推进到 `current=5120/highest=25268023`；still does not claim full discv4 peer churn or long-haul mainnet acceptance
