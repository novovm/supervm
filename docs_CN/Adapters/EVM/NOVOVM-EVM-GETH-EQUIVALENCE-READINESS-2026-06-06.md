...
- snap/1 AccountRange -> StorageRanges/ByteCodes follow-up gate: pass, covers non-empty slim account response -> storage root / code hash extraction -> real `GetStorageRanges` + `GetByteCodes` follow-up requests -> matched responses clear snap pending state；still does not claim storage/code persistence, proof verification, or full trie heal
