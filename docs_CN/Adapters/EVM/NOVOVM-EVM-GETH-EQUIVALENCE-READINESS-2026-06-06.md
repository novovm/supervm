...
- RLPx BlockHeaders continuity gate: pass, covers inbound `BlockHeaders` batch validation before native import；相邻 header 必须 `number + 1` 连续且子 header `parentHash` 等于上一 header hash，编号跳跃或拼接批次按 peer decode failure 拒绝
...
cargo test -p novovm-network rlpx_block_headers_validation_rejects_non_contiguous_batch_v1 -- --nocapture
