...
46. 2026-06-10 current-head body-first 收口：继续只修 RLPx 长期同步链路。24 tick 真实复验从 `25282286` 推进到 `25282360`，但暴露 near-head 大 batch 的本地策略缺口：当 `GetBlockHeaders` 一次返回到已知 `highest` 时，旧逻辑认为“不是追高”，于是立即对 20-60 个 headers 发送大批量 `GetBlockBodies`；公网 peer 多轮 partial body retry 后断开，当前 head 反而需要后续 body recovery 才补齐。本轮把 header follow-up body 选择改为：大于 16 的 header batch 即使刚好到达当前 known highest，也只优先请求最新/current head body；全部 header 仍写入 canonical/history，历史 body 缺口留给 bounded missing-body recovery。另一个同轮修复是：当前 head 已完整且 `current==highest` 时，历史 backfill 的 transport failure 不再回退 forward header window，避免纯 backfill 失败惩罚下一轮追高。回归 `cargo test -p novovm-network rlpx_large_header_batch_import_requests_current_body_only_at_highest_v1 -- --nocapture`、`cargo test -p novovm-network rlpx_header_batch_import_requests_current_body_only_while_chasing_v1 -- --nocapture`、`cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1`、`cargo test -p novovm-node eth_rlpx_idle_backfill_failure_does_not_backoff_header_window_v1 -- --nocapture`、`cargo test -p novovm-node eth_rlpx_ -- --nocapture`、`cargo check -p novovm-network -p novovm-node` 通过。真实入口 12 tick 从完整 `25282380/highest=25282380` 附近启动，tick 1 发出 `GetBlockHeaders max=19/20`，随后只出现 `bodies_requested blocks=1` 的 current-head follow-up，tick 2 推进到完整 `25282380`，后续继续推进到 `25282401` 但最新 head receipt 尚待 recovery。剩余仍是公网 `too_many_peers`、timeout、pre-auth close、current-head receipt recovery 和 24h 重启续跑验证，不声明 geth-like 长期主网同步完成。
