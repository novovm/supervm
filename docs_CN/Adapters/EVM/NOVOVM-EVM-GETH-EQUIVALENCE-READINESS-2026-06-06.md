...
这证明 adapter 在拿到 sender account pre-state 时，会执行生产级 value/fee debit 和余额不足拒绝；没有 sender account pre-state 的 plugin smoke 仍保持控制面执行，不伪造余额。当前已贯通 raw / gateway access-list entries 到 TxIR，并能把 declared storage read 写入 observed BAL；已补最小 warm/cold 成本和 BAL 执行观测门禁，但仍未跑 Ethereum execution-spec 官方 warm/cold fixture，因此不声明完整 EIP-2929 warm set 等价。
...
| Ethereum fork rules / gas accounting / precompiles | Partial | execution-spec/fork-rule smoke matrix pass；adapter balance/fee/access-storage smoke pass；access-list entries 贯通 smoke pass；access-list warm/cold 成本和 BAL smoke pass；未跑 Ethereum execution-spec 全量 fixture | 可声明样本级 fork-rule、gas、tracked-account fee/value debit、access-list read-set/warm-cold smoke/BAL gate；不能声明 EVM 语义全等价 |
...
1. 如要继续提高执行语义置信度，接入 Ethereum execution-spec 官方 fixture 子集，优先选账户余额、gas refund、SSTORE/SLOAD refund、CREATE/CALL failure 样本。
2. 如要完整验证 access-list warm/cold 语义，基于现在已贯通的 `TxIR.evm_access_list` 和最小 warm/cold smoke 接官方 fixture；不要再做包装层。
