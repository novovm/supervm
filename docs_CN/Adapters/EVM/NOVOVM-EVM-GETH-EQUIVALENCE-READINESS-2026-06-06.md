...
- novovm-node direct RLPx sync entry: pass for finite live mainnet run via `NOVOVM_NODE_MODE=eth_rlpx_sync`, covers real Status -> Headers -> Bodies -> Receipts and native current advancing from 0 to 3072；默认候选池已从 Ethereum mainnet geth bootnodes 扩展到 geth DNS discovery `all.mainnet.ethdisco.net` ENR 解析，实测候选从 4 扩到 64；still does not claim DNS tree signature verification, full discv4 peer churn, or long-haul catch-up
- RLPx remote-best sticky target gate: pass, 75 tick live soak 暴露断线后 `highest` 回落到 local current 的问题；runtime sync status 已增加 5 分钟 remote-best hint，后续 20 tick live run 中断线后 `highest` 保持远端高度、不再丢失追高目标（实测 `current=3072` 时 `highest=25267430`）；still does not claim full long-haul catch-up
