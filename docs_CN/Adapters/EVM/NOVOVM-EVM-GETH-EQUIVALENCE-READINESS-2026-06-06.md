...
- novovm-node RLPx public sync batch gate: pass, `eth_rlpx_sync` 产品入口默认把公网 `GetBlockHeaders`/`GetBlockBodies` 批量收敛到 `NOVOVM_ETH_RLPX_HEADERS_BATCH=128`、`NOVOVM_ETH_RLPX_BODIES_BATCH=8`，减少 churny public peer 上的大 frame 中途 EOF 压力；该收敛只作用于产品入口 worker budget，不改变底层 native fullnode 默认 2048/256 能力
