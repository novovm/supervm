...
- EVM gateway Engine API probe gate: pass, `engine_exchangeCapabilities` 已从 standalone EVM control namespace 禁用列表里单独放行，返回当前真实支持的 `["engine_exchangeCapabilities"]`；`engine_getPayloadV3` 等 payload/forkchoice 控制方法仍保持禁用并映射为 method not found，避免在没有真实 execution/forkchoice 语义前向共识层伪造支持；still does not claim CL-driven mainnet forkchoice or full Engine API
