...
- RLPx NewBlock gate: pass, covers inbound real `NewBlock` announcement -> empty body `transactionsRoot`/`receiptsRoot`/ommers hash validation -> native header/body snapshot import -> peer head/highest update -> follow-up `GetReceipts`
...
RLPx 主网同步方面，当前已新增 `NewBlock` / `NewBlockHashes` 公告处理、pooled tx hash/request/response 链路和 receipts wire-level 同步链路：真实 peer 发出新头公告后，SUPERVM 会更新 peer head/highest 并主动发起后续同步；`NewBlock` 空交易 body 会先校验 Ethereum empty trie root、empty ommers hash 和可见的 empty withdrawals root，不再无条件导入明显错误的空块；`BlockBodies` 或 `NewBlock` 返回后会继续发 eth/70 `GetReceipts(firstBlockReceiptIndex=0)`，收到 `Receipts` 后才把本轮 peer sync 标记 ready；`NewPooledTransactionHashes` 会触发 `GetPooledTransactions` 并 materialize raw tx payload，本地 pending raw tx 也能响应远端 `GetPooledTransactions`。本地响应 `GetReceipts` 时只对可证明的空交易 body 返回空 receipts，不伪造有交易块的 receipt 数据。这仍不是完整 geth peer 行为；完整 receipt store、snap state sync、非空 block root/state execution validation、eth/71 完整协商和长稳主网 soak 仍未封口。
...
| devp2p/RLPx peer sync / block import | Partial | 真实 RLPx handshake/Status + 入站 `Transactions` -> native pending tx raw RLP gate pass；出站 `Transactions` broadcast gate pass；pooled tx hash/request/response gate pass；block header/body/receipts sync gate pass；最小 reorg 回池 gate pass；`NewBlock` / `NewBlockHashes` gates pass；`NewBlock` 空 body root validation pass；未覆盖完整 receipt store、snap state sync、非空 block root/state execution validation、完整 eth/71 peer sync、长稳主网接受度和复杂多分支 reorg | 可声明最小 RLPx tx propagation/pooled-tx/header-body-receipts import/reorg 回池/NewBlock/NewBlockHashes 和空 body root validation 样本可观察；不能声明以太坊全节点 |
