...
- geth BAL validation recheck gate: on 2026-06-09, the same geth HEAD also includes `core/types/bal: improve the bal validation (#35110)`, which rejects BAL `storageChanges` entries with empty `slotChanges`. SUPERVM now applies that check to raw `BlockAccessLists` payloads received over RLPx before storing them, and additionally rejects BAL payloads whose raw-RLP hash does not match the header `BlockAccessListHash`.
