...
- node Ethereum DNS startup budget gate: `cargo test -p novovm-node eth_dns_discovery_default_max_queries_is_startup_bounded_v1 -- --nocapture` and `cargo test -p novovm-node eth_dns_query_txt_respects_expired_discovery_deadline_v1 -- --nocapture` (`NOVOVM_ETH_DNS_DISCOVERY_TOTAL_TIMEOUT_MS` bounds DNS tree walk startup time, default `NOVOVM_ETH_DNS_DISCOVERY_MAX_QUERIES` scales with requested candidates instead of scanning 512 records, and each DoH TXT query uses the remaining global discovery deadline)
