...
- node RLPx public sync batch gate: `cargo test -p novovm-node eth_rlpx_public_sync_batch_defaults_are_product_chase_ready_v1 -- --nocapture` (`novovm-node` public RLPx entry defaults to `NOVOVM_ETH_RLPX_HEADERS_BATCH=128` and `NOVOVM_ETH_RLPX_BODIES_BATCH=32`, improving product-entry forward chase while still allowing operators to lower the body batch by env on churny public peers; the lower-level native fullnode defaults remain wider at 2048/256)
- node RLPx public sync batch-32 live validation: product entry `NOVOVM_NODE_MODE=eth_rlpx_sync` with default env continued from `current=1469/highest=25275734`; after peer refresh expanded candidates to 268, tick 7 imported `headers=32/bodies=32/receipts=32` and advanced to `current=1501`, then a second 32-header step reached `current=1533`; a temporary header-only head at `1533` recovered by tick 12 back to current body/receipt availability, so the faster default keeps the existing missing-body recovery path viable under public peer churn.
...
- node RLPx stalled peer refresh gate: `cargo test -p novovm-node eth_rlpx_ -- --nocapture` and `cargo test -p novovm-node eth_peer_endpoint_ -- --nocapture` (when public peers churn and no ready peer is available while `highest > current`, or startup has no ready peer and no remote highest yet, the product entry expands/refreshes candidates by `NOVOVM_ETH_RLPX_STALLED_REFRESH_INTERVAL_TICKS` default 4 ticks; refreshed endpoints are merged with the existing pool so refresh does not shrink candidates; initial candidates now accept up to 512 and adaptive candidates now default to at least 512 and can go up to 1024, while active peers remain capped by `NOVOVM_ETH_RLPX_MAX_PEERS` default 32; prior max-peers 16 live runs advanced from `current=973/highest=25274693` to `current=1277/highest=25275268`, with partial body frame closes recovered back to current body/receipts available, including blocks `1213` and `1269` after mid-frame remote closes; the follow-up run from `current=1277/highest=25275268` expanded candidates to 356 but stayed ready=0 through tick 15 under 16 active peers, so the product default now uses the existing 32-peer cap to improve public mainnet admission while staying below local geth `MaxPeers=50`; default32 live validation reached ready at tick 2/11, advanced to `current=1293/highest=25275386`, and recovered block `1293` body/receipt after a mid-body remote close)
- geth update review gate: local `D:\WEB3_AI\go-ethereum` pulled to `1f87331fb` (`eth/protocols/eth: track announced tx hashes only after send`); the only network-observable change in the latest batch is geth marking `NewPooledTransactionHashes` known only after a successful send. SUPERVM does not have a product outbound hash-only announce path yet, and its current outbound `TransactionsMsg` broadcast records propagation only after wire-frame write success. RLPx live session write/ingest failures now unregister the peer and remove the live session before returning, so a mid-frame public-peer close can be retried by the next ready peer. Mainnet product-entry validation from restored `current=1301` recovered `1301` body/receipt, then recovered a new `1309` header-only gap and advanced to `current=1317/highest=25275535` with current body/receipts available.
- geth re-pull review gate: on 2026-06-09, `D:\WEB3_AI\go-ethereum` `git pull --ff-only` returned `Already up to date`; local/remote `origin/master` remain `1f87331fb`. Re-reading the latest diff confirmed it only moves geth `sendPooledTransactionHashes` known-tx marking after successful `NewPooledTransactionHashes` send. SUPERVM still has no product outbound hash-only announce path; its outbound full `TransactionsMsg` path records propagation only after `eth_rlpx_write_wire_frame_v1` succeeds and records failure on write error, so no Rust code sync is required for this geth recheck.
- geth re-pull no-new-upstream gate: on 2026-06-09, `D:\WEB3_AI\go-ethereum` `git fetch --prune origin` showed `HEAD...origin/master = 0 0`, and `git pull --ff-only` again returned `Already up to date`; current geth remains `1f87331fb`. The tx gossip impact review is unchanged: geth `#35122` only delays known-tx marking for hash-only pooled announcements until send success, while SUPERVM currently propagates product outbound txs as full `TransactionsMsg` and records propagated only after successful wire-frame write, with write failures recorded as propagation failures.
...
- node Ethereum DNS startup budget gate: `cargo test -p novovm-node eth_dns_discovery_default_max_queries_is_startup_bounded_v1 -- --nocapture` and `cargo test -p novovm-node eth_dns_query_txt_respects_expired_discovery_deadline_v1 -- --nocapture` (`NOVOVM_ETH_DNS_DISCOVERY_TOTAL_TIMEOUT_MS`, default 5s, bounds DNS tree walk startup time so the 10s peer discovery budget still leaves time for discv4; default `NOVOVM_ETH_DNS_DISCOVERY_MAX_QUERIES` scales with requested candidates instead of scanning 512 records, and each DoH TXT query uses the remaining global discovery deadline)
- node Ethereum peer discovery total budget gate: `cargo test -p novovm-node eth_rlpx_peer_discovery_deadline_caps_phase_timeout_v1 -- --nocapture` (`NOVOVM_ETH_RLPX_PEER_DISCOVERY_TOTAL_TIMEOUT_MS`, default 10s, caps DNS+discv4 startup/refresh discovery before the product entry falls back to already discovered endpoints plus geth bootnodes and enters RLPx ticks; 128-candidate bounded probe entered tick in 10.5s and reached a ready peer)
