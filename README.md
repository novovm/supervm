...
- v3 RLPx response request-match gate: `cargo test -p novovm-network rlpx_ -- --nocapture --test-threads=1` (`PooledTransactions`/`BlockBodies`/`Receipts`/`BlockAccessLists` responses require a matching pending request before materialization; pooled tx responses must be an ordered subset of requested hashes)
