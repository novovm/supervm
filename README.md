...
- network RLPx head-first body recovery gate: `cargo test -p novovm-network rlpx_missing_body_recovery_ -- --nocapture --test-threads=1` (while chasing remote head, missing-body recovery now requests the current header-only head first and carries up to 16 contiguous header-only hashes instead of the old 4-block suffix. This lets short public-peer `BlockBodies` responses materialize the current head before older parents. Live validation from persisted header-only `25279055` emitted `missing_bodies_requested ... blocks=16` to multiple ready peers; no bodies were returned under public `too_many_peers`/ingest-close churn, so this closes request ordering/window size only, not body-serving peer availability or 24h long-sync.)
