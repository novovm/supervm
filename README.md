...
- node RLPx native restore phase gate: `cargo test -p novovm-node eth_rlpx_native_ -- --nocapture` (latest head/history stores restore validated header/body/receipt snapshots and resume runtime head at `State` when receipts are available, instead of falling back to `Headers`; latest head store also persists/restores bounded snap account/storage/code subsets for the restored head `stateRoot`)
