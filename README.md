...
- node native Ethereum RLPx sync entry: `NOVOVM_NODE_MODE=eth_rlpx_sync cargo run -p novovm-node --bin novovm-node` (direct Rust node entry; uses `NOVOVM_ETH_RLPX_ENODES`, `NOVOVM_ETH_RLPX_CHAIN_ID`, `NOVOVM_ETH_RLPX_TICKS`, `NOVOVM_ETH_RLPX_RECV_BUDGET`, `NOVOVM_ETH_RLPX_CANDIDATE_PEERS`, `NOVOVM_ETH_RLPX_SYNC_TARGET_FANOUT`; default candidate order is explicit ENODEs first, then geth DNS discovery `all.mainnet.ethdisco.net` + discv4-discovered peers, with Ethereum mainnet geth bootnodes only as direct-connect fallback; default candidate pool is 256, active peers default to `NOVOVM_ETH_RLPX_MAX_PEERS=32`, per-tick sync/bootstrap fanout starts at 8 for fast tick visibility, and admission-stalled public runs auto-raise fanout to the active window unless `NOVOVM_ETH_RLPX_SYNC_TARGET_FANOUT` is explicitly set)
...
- node RLPx bootstrap tick budget gate: `cargo test -p novovm-network real_rlpx_bootstrap_tick_budget_defers_slow_connects_v1 -- --nocapture` (real worker caps per-tick bootstrap wall clock by `NOVOVM_ETH_RLPX_BOOTSTRAP_TICK_BUDGET_MS`, default `12000`, clamped `1000..60000`, and reports `skipped_bootstrap_budget_peers`/`skippedBootstrapBudgetPeers` so adaptive 32-fanout public runs defer slow TCP/auth peers instead of blocking tick output for minutes; live product entry with `NOVOVM_ETH_RLPX_TICKS=3` and `NOVOVM_ETH_RLPX_SYNC_TARGET_FANOUT=32` returned in 58s, reached ready peers on ticks 1/2, and recovered body/receipts for `current=1981`)
...
- geth public request-timeout review gate: on 2026-06-09, official geth remained at `1f87331fb`, but re-reading `eth/downloader/*` and `p2p/msgrate/msgrate.go` confirmed geth uses dynamic `TargetTimeout()` for header/body/snap requests (`ttlScaling=3`, capped by `ttlLimit=1m`) instead of a fixed 5s public-peer deadline. SUPERVM's direct product node entry now defaults `NOVOVM_ETH_RLPX_REQUEST_TIMEOUT_MS=15000` for public RLPx sync, with `1000..120000` env clamp, while the core library default stays unchanged. Live product entry validation from `current=1853/highest=25277653` reached ready peers at ticks 5/7 and advanced known `highest` to `25277702` without the prior `rlpx_request_timeout:headers`; the remaining failure class was remote public-peer close / mid-frame read failure (`rlpx_frame_body_read_failed read=61696/171088`), not a request TTL mismatch.
- node RLPx adaptive batch gate: `cargo test -p novovm-node eth_rlpx_ -- --nocapture` now covers public sync batch backoff/restore (`22 passed`). The product entry still starts with geth-like `headers=192`/`bodies=128`, but `NOVOVM_ETH_RLPX_ADAPTIVE_BATCH_ENABLED` defaults on and temporarily halves headers/bodies batches down to `NOVOVM_ETH_RLPX_ADAPTIVE_HEADERS_MIN_BATCH` default `64` and `NOVOVM_ETH_RLPX_ADAPTIVE_BODIES_MIN_BATCH` default `32` when public peers close or timeout an active sync request; any real header/body/receipt progress restores batches upward. Live validation from `current=1853/highest=25277702` hit `rlpx_frame_body_read_failed read=52276/171088` at tick 10 and immediately emitted `eth_rlpx_adaptive_batch ... headers_old=192 headers_new=96 bodies_old=128 bodies_new=64`, so long-running product sync no longer retries the same oversized request window after a mid-frame public-peer reset.
