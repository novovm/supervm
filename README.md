...
- v3 RLPx header continuity gate: `cargo test -p novovm-network rlpx_block_headers_validation_rejects_non_contiguous_batch_v1 -- --nocapture` (real `BlockHeaders` batches must be internally contiguous by `number` and `parentHash` before native header materialization; gap/spliced batches are rejected as peer decode failures)
