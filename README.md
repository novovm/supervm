...
- node RLPx stalled peer refresh gate: `cargo test -p novovm-node eth_rlpx_peer_refresh_plan -- --nocapture` (when public peers churn and no ready peer is available while `highest > current`, the product entry expands/refreshes candidates by `NOVOVM_ETH_RLPX_STALLED_REFRESH_INTERVAL_TICKS`; initial candidates now accept up to 512 and adaptive candidates up to 1024, while active peers remain capped by `NOVOVM_ETH_RLPX_MAX_PEERS`)
